# AI Security Repository Radar

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Inter&weight=700&size=30&pause=1000&color=00D4FF&width=900&lines=AI+Security+Radar;LLM+Security+%7C+MCP+Security+%7C+RAG+Security;AI+Labs+%7C+ML+Labs+%7C+MCP+Labs;Automatically+Updated+Daily)](https://git.io/typing-svg)

![Repos](https://img.shields.io/badge/repos-506-blue) ![Stars](https://img.shields.io/badge/stars-226406-yellow) ![Forks](https://img.shields.io/badge/forks-31254-orange) ![Labs](https://img.shields.io/badge/labs-58-purple) ![Trusted](https://img.shields.io/badge/trusted_sources-31-success) ![Updated](https://img.shields.io/badge/updated-daily-brightgreen) ![Maintained](https://img.shields.io/badge/maintained-yes-success)

> Automatically discovered and categorized GitHub repositories focused on AI security, LLM security, MCP security, RAG security, agent security, AI red teaming, model security, ML security labs, and AI supply chain security.

**Last updated:** `2026-09-26T13:03:16.067598+00:00`

---

## 🚀 Executive Summary

- 🏆 **Most starred repo:** [The-Art-of-Hacking/h4cker](https://github.com/The-Art-of-Hacking/h4cker) with **29,537** stars
- 💎 **Highest quality signal:** [RiccardoBiosas/awesome-MLSecOps](https://github.com/RiccardoBiosas/awesome-MLSecOps) with quality score **120**
- 🔥 **Top 24h mover:** [The-Art-of-Hacking/h4cker](https://github.com/The-Art-of-Hacking/h4cker) with **+0** stars today
- 🆕 **New repos discovered:** **506**
- 🧪 **Labs detected:** **58**
- ✅ **Trusted-source repos:** **31**
- 📦 **Largest category:** **MCP Security**
- 📈 **Fastest growing category:** **AI Governance / Risk** based on 7-day star growth

---

## 📊 Overview

| Metric | Value |
|---|---:|
| Total repositories | 506 |
| Total stars tracked | 226,406 |
| Total forks tracked | 31,254 |
| Trending repositories | 0 |
| New repositories | 506 |
| Labs | 58 |
| Trusted-source repos | 31 |
| Categories | 10 |

---

## 🧭 Quick Navigation

- [🚀 Executive Summary](#-executive-summary)
- [📊 Overview](#-overview)
- [🔥 Trending Repositories](#-trending-repositories)
- [🆕 New Repositories](#-new-repositories)
- [🧪 Labs](#-labs)
- [💎 Highest Quality Signals](#-highest-quality-signals)
- [🏆 Top Repositories](#-top-repositories)
- [📦 Category Summary](#-category-summary)
- [🗂️ Repository Categories](#️-repository-categories)

  - [⚖️ AI Governance / Risk](#ai-governance-risk)
  - [⚔️ AI Red Teaming](#ai-red-teaming)
  - [📦 AI Supply Chain Security](#ai-supply-chain-security)
  - [🤖 Agent Security](#agent-security)
  - [🛡️ LLM Application Security](#llm-application-security)
  - [🧪 Labs](#labs)
  - [🧩 MCP Security](#mcp-security)
  - [🧬 Model / ML Security](#model-ml-security)
  - [🔐 Other AI Security](#other-ai-security)
  - [📚 RAG Security](#rag-security)

---

## 🔥 Trending Repositories

No trending repositories yet. This is normal during the first few daily runs.

---

## 🆕 New Repositories

### 🆕 🏆 💎 [The-Art-of-Hacking/h4cker](https://github.com/The-Art-of-Hacking/h4cker)

This repository is maintained by Omar Santos (@santosomar) and includes thousands of resources related to ethical hacking, bug bounties, digital forensics and incident response (DFIR), AI security, vulnerability resea...

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** Tool / Function Calling
- **Type:** `Awesome List`
- **Stars:** 29,537
- **Forks:** 5,384
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Jupyter Notebook`
- **License:** `MIT`
- **Source trust:** `Community`
- **Relevance score:** `19`
- **Quality score:** `59`
- **Topics:** ai, ai-security, artificial-intelligence, awesome-list, awesome-lists, cybersecurity, ethical-hacking, exploit

### 🆕 [Elite588/Hacker](https://github.com/Elite588/Hacker)

This repository is maintained by Hex Core and includes thousands of resources related to ethical hacking, bug bounties, digital forensics and incident response (DFIR), AI security, vulnerability research, exploit deve...

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** Tool / Function Calling
- **Type:** `Awesome List`
- **Stars:** 106
- **Forks:** 2
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Jupyter Notebook`
- **License:** `MIT`
- **Source trust:** `Community`
- **Relevance score:** `20`
- **Quality score:** `31`
- **Topics:** ai, ai-safety, artificial-intelligence, awesome, awesome-list, cybersecurity, ethical-hacking, exploit

### 🆕 [ZeroDayEvil/CVE-2026-54121-Certighost](https://github.com/ZeroDayEvil/CVE-2026-54121-Certighost)

🛡️ Official AI Security Tool module for CVE-2026-54121 (Certighost - AD CS Domain Controller Impersonation & PKINIT Elevation).

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** Dashboard / Platform
- **Type:** `Tool / Framework`
- **Stars:** 83
- **Forks:** 0
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Unknown`
- **License:** `Unknown`
- **Source trust:** `Community`
- **Relevance score:** `19`
- **Quality score:** `29`
- **Topics:** active-directory, ai-security-tool, cve-2026-54121, privilege-escalation, red-team

### 🆕 [socprime/AIDR-Bastion](https://github.com/socprime/AIDR-Bastion)

A comprehensive GenAI protection system designed to protect against malicious prompts, injection attacks, and harmful content. System incorporates multiple engines that operate in sequence to analyze and classify user...

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** RAG Poisoning
- **Type:** `Tool / Framework`
- **Stars:** 106
- **Forks:** 6
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Python`
- **License:** `LGPL-3.0`
- **Source trust:** `Community`
- **Relevance score:** `17`
- **Quality score:** `28`
- **Topics:** none

### 🆕 [ZeroDayEvil/CVE-2026-41096-PoC](https://github.com/ZeroDayEvil/CVE-2026-41096-PoC)

🛡️ Official AI Security Tool module for CVE-2026-41096 (Windows DNSAPI.dll Heap Overflow / DNS-Mayhem Deep Dive Analysis & Audit Module).

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** Dashboard / Platform
- **Type:** `Research / Benchmark`
- **Stars:** 92
- **Forks:** 0
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Unknown`
- **License:** `Unknown`
- **Source trust:** `Community`
- **Relevance score:** `17`
- **Quality score:** `27`
- **Topics:** ai-security-tool, cve-2026-41096, dnsapi, heap-overflow, windows-security

### 🆕 [ZeroDayEvil/CVE-2026-20805-PoC](https://github.com/ZeroDayEvil/CVE-2026-20805-PoC)

🛡️ Official AI Security Tool module for CVE-2026-20805 (Desktop Window Manager / dwm.exe Information Disclosure & Memory Leak Diagnostic).

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** Dashboard / Platform
- **Type:** `Tool / Framework`
- **Stars:** 86
- **Forks:** 1
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Unknown`
- **License:** `Unknown`
- **Source trust:** `Community`
- **Relevance score:** `16`
- **Quality score:** `26`
- **Topics:** ai-security-tool, cve-2026-20805, windows-security

### 🆕 [ZeroDayEvil/CVE-2026-21858-n8n-FullChain](https://github.com/ZeroDayEvil/CVE-2026-21858-n8n-FullChain)

🛡️ Official AI Security Tool module for CVE-2026-21858 + CVE-2025-68613 (n8n "Ni8mare" Unauthenticated Arbitrary File Read & Expression Injection RCE Full Chain).

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** Sandboxing
- **Type:** `Tool / Framework`
- **Stars:** 86
- **Forks:** 0
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Unknown`
- **License:** `Unknown`
- **Source trust:** `Community`
- **Relevance score:** `16`
- **Quality score:** `26`
- **Topics:** ai-security-tool, arbitrary-file-read, cve-2025-68613, cve-2026-21858

### 🆕 [laboratoriohacker-com/tatu](https://github.com/laboratoriohacker-com/tatu)

Free, open source, self-hosted. Built by Laboratório Hacker for teams that take AI security seriously. Deploy in minutes.

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** Dashboard / Platform
- **Type:** `Tool / Framework`
- **Stars:** 36
- **Forks:** 2
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Python`
- **License:** `Unknown`
- **Source trust:** `Community`
- **Relevance score:** `11`
- **Quality score:** `21`
- **Topics:** none

### 🆕 [jeanmalaquias/ai-governance-mapping](https://github.com/jeanmalaquias/ai-governance-mapping)

Unified AI governance controls catalog crosswalking NIST AI RMF 1.1, EU AI Act, OWASP LLM Top 10, and ISO/IEC 42001 - with OPA policies, templates, and a compliance gap-check CLI.

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** Guardrails
- **Type:** `Research / Benchmark`
- **Stars:** 13
- **Forks:** 3
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Python`
- **License:** `MIT`
- **Source trust:** `Community`
- **Relevance score:** `6`
- **Quality score:** `16`
- **Topics:** none

### 🆕 [jiangnanboy/llm_security](https://github.com/jiangnanboy/llm_security)

利用分类法和敏感词检测法对生成式大模型的输入和输出内容进行安全检测，尽早识别风险内容。The input and output contents of generative large model are checked by classification method and sensitive word detection method to identify content risk as early as possible.

- **Category:** ⚖️ AI Governance / Risk
- **Subcategory:** General
- **Type:** `Project`
- **Stars:** 28
- **Forks:** 3
- **Growth:** +0 stars / 1d, +0 stars / 7d
- **Language:** `Java`
- **License:** `MIT`
- **Source trust:** `Community`
- **Relevance score:** `10`
- **Quality score:** `12`
- **Topics:** llm, llm-security

---

## 🧪 Labs

### LLM Labs

| Lab | Description | Category | Stars | Growth | Language | Score |
|---|---|---|---:|---:|---|---:|
| 🆕 💎 🧠 🧪 [AISecurityConsortium/AIGoat](https://github.com/AISecurityConsortium/AIGoat) | AIGoat - Open-source AI security playground for LLM red teaming. AI Goat provides hands-on labs covering the full OWA... | ⚔️ AI Red Teaming | 93 | +0 / +0 | `JavaScript` | 92 |
| 🆕 💎 🧠 🧪 [Mr-Infect/AI-penetration-testing](https://github.com/Mr-Infect/AI-penetration-testing) | AI/ML/LLM Penetration Testing Toolkit by Mr-Infect — the #1 GitHub resource for AI security, red teaming, and adversa... | 🧬 Model / ML Security | 288 | +0 / +0 | `Unknown` | 84 |
| 🆕 💎 🧠 🧪 [regaan/basilisk](https://github.com/regaan/basilisk) | Basilisk — Open-source AI red teaming framework with genetic prompt evolution. Automated LLM security testing for GPT... | 🧬 Model / ML Security | 31 | +0 / +0 | `Python` | 84 |
| 🆕 💎 🧠 🧪 [CyberSunil/LLMVault](https://github.com/CyberSunil/LLMVault) | An intentionally vulnerable OWASP LLM Top 10 training platform for AI Security, Prompt Injection, RAG Security, Agent... | 🧬 Model / ML Security | 326 | +0 / +0 | `Python` | 82 |
| 🆕 💎 🧠 🧪 ✅ [microsoft/AI-Red-Teaming-Playground-Labs](https://github.com/microsoft/AI-Red-Teaming-Playground-Labs) | AI Red Teaming playground labs to run AI Red Teaming trainings including infrastructure. | 🧬 Model / ML Security | 2,080 | +0 / +0 | `TypeScript` | 81 |
| 🆕 💎 🧠 🧪 [Panda1847/ai-security-lab](https://github.com/Panda1847/ai-security-lab) | 🤖 Advanced AI/LLM Security Testing Framework. Includes 50+ jailbreak techniques, prompt injection tools, and automate... | 🧬 Model / ML Security | 10 | +0 / +0 | `Unknown` | 80 |
| 🆕 💎 🧠 🧪 [schwartz1375/genai-security-training](https://github.com/schwartz1375/genai-security-training) | No description. | 🧬 Model / ML Security | 461 | +0 / +0 | `Jupyter Notebook` | 76 |
| 🆕 💎 🧠 🧪 [airtasystems/DVAIA-Damn-Vulnerable-AI-Application](https://github.com/airtasystems/DVAIA-Damn-Vulnerable-AI-Application) | Damn Vulnerable AI Application - For LLM Red Team Training. LLM testing, RAG testing, Multimodal testing, Agent testi... | 📚 RAG Security | 41 | +0 / +0 | `Python` | 67 |
| 🆕 💎 🧠 🧪 [0xSojalSec/Awesome-AI-Security](https://github.com/0xSojalSec/Awesome-AI-Security) | Resources, research, and tools for securing AI systems | 🧬 Model / ML Security | 31 | +0 / +0 | `Unknown` | 65 |
| 🆕 💎 🧠 🧪 [Masriyan/Claude-Code-CyberSecurity-Skill](https://github.com/Masriyan/Claude-Code-CyberSecurity-Skill) | 22 production-quality Claude Code Skills for cybersecurity professionals — covering offensive security, defensive ope... | ⚔️ AI Red Teaming | 446 | +0 / +0 | `Python` | 64 |
| 🆕 💎 🧠 🧪 [user1342/Awesome-LLM-Red-Teaming](https://github.com/user1342/Awesome-LLM-Red-Teaming) | A curated list of awesome LLM Red Teaming training, resources, and tools. | ⚔️ AI Red Teaming | 135 | +0 / +0 | `Unknown` | 62 |
| 🆕 💎 🧠 🧪 [ErdemOzgen/RedAiRange](https://github.com/ErdemOzgen/RedAiRange) | A professional AI security range for red teaming, vulnerability research, defensive validation, and hands-on AI/ML se... | 🧬 Model / ML Security | 130 | +0 / +0 | `Jupyter Notebook` | 60 |
| 🆕 💎 🧠 🧪 [dhammon/ai-goat](https://github.com/dhammon/ai-goat) | Learn AI security through a series of vulnerable LLM CTF challenges. No sign ups, no cloud fees, run everything local... | 🛡️ LLM Application Security | 366 | +0 / +0 | `Python` | 57 |
| 🆕 💎 🧠 🧪 [securelayer7/PROMPTPurify](https://github.com/securelayer7/PROMPTPurify) | Prompt-injection guardrail for LLM applications. Compact model that outperforms larger open-source guards. No regex, ... | 🛡️ LLM Application Security | 80 | +0 / +0 | `TypeScript` | 56 |
| 🆕 💎 🧠 🧪 [Esther7171/TryHackMe-Walkthroughs](https://github.com/Esther7171/TryHackMe-Walkthroughs) | 137+ TryHackMe walkthroughs covering web exploitation, privilege escalation, CVE analysis, container escapes, and AI/... | 🛡️ LLM Application Security | 79 | +0 / +0 | `Python` | 54 |
| 🆕 💎 🧠 🧪 [anpa1200/AI-PT-Lab](https://github.com/anpa1200/AI-PT-Lab) | Intentionally vulnerable AI security training lab for LLM agents — demonstrates OWASP LLM Top 10 2025: RAG injection,... | 🛡️ LLM Application Security | 12 | +0 / +0 | `Python` | 54 |
| 🆕 💎 🧠 🧪 [aligorithm/Zero-Health](https://github.com/aligorithm/Zero-Health) | Zero trust. Zero security. Total exposure. A deliberately vulnerable health tech platform with AI Chatbot for learnin... | 🧬 Model / ML Security | 48 | +0 / +0 | `JavaScript` | 52 |
| 🆕 💎 🧠 🧪 [KryptSec/oasis](https://github.com/KryptSec/oasis) | Open-source AI security benchmarking CLI. Measure how AI models perform offensive security tasks with MITRE ATT&CK an... | 🤖 Agent Security | 29 | +0 / +0 | `TypeScript` | 52 |
| 🆕 🧠 🧪 [wearetyomsmnv/Awesome-LLM-agent-Security](https://github.com/wearetyomsmnv/Awesome-LLM-agent-Security) | All about llm-agents security,attack,vulnerabilities and how to do them for cybersecurity. | 📚 RAG Security | 58 | +0 / +0 | `Unknown` | 47 |
| 🆕 🧠 🧪 ✅ [OWASP/www-project-promptme](https://github.com/OWASP/www-project-promptme) | PromptMe is an educational project that showcases security vulnerabilities in large language models (LLMs) and their ... | 🧪 Labs | 39 | +0 / +0 | `Python` | 47 |
| 🆕 🧠 🧪 [TrustAI-laboratory/LLM-Security-CTF](https://github.com/TrustAI-laboratory/LLM-Security-CTF) | Learn LLM/AI Security through a series of vulnerable LLM CTF challenges. No sign ups, all fees, everything on the web... | 🛡️ LLM Application Security | 20 | +0 / +0 | `Unknown` | 47 |
| 🆕 🧠 🧪 [R3dShad0w7/PromptMe](https://github.com/R3dShad0w7/PromptMe) | PromptMe is an educational project that showcases security vulnerabilities in large language models (LLMs) and their ... | ⚔️ AI Red Teaming | 111 | +0 / +0 | `Python` | 46 |
| 🆕 🧠 🧪 [gatsby-sec/llm-sec-range](https://github.com/gatsby-sec/llm-sec-range) | 🛡️ 大模型攻防渗透测试靶场 · 提示注入CTF / OWASP LLM Top10 / 脆弱Agent / 资料聚合。目标模型可切换：DeepSeek直连、OpenRouter中转站(国产+国外十余款小模型)、本地Ollama(De... | ⚔️ AI Red Teaming | 30 | +0 / +0 | `Python` | 46 |
| 🆕 🧠 🧪 [0x4D31/airt](https://github.com/0x4D31/airt) | AIRT — A free, open-source AI Red Teaming course with 8 modules and hands-on Docker labs. Built with Perplexity Compu... | 🧬 Model / ML Security | 230 | +0 / +0 | `HTML` | 45 |
| 🆕 🧠 🧪 [r00tb3/RAG-Poisoning-Lab](https://github.com/r00tb3/RAG-Poisoning-Lab) | RAG Poisoning Lab — Educational AI Security Exercise | 📚 RAG Security | 10 | +0 / +0 | `Python` | 44 |
| 🆕 🧠 🧪 [Tcotl/DVLAA](https://github.com/Tcotl/DVLAA) | DVLAA (Damn Vulnerable LLM and Agent Application) is a local LLM and Agent security range for OWASP LLM Top 10 and Ag... | 🤖 Agent Security | 33 | +0 / +0 | `Python` | 43 |
| 🆕 🧠 🧪 [randalltr/prompt-engineering-for-hackers](https://github.com/randalltr/prompt-engineering-for-hackers) | Prompt Engineering for Hackers: A Hands-On Intro to LLMs, Jailbreaks, and Adversarial Prompting | ⚔️ AI Red Teaming | 11 | +0 / +0 | `Unknown` | 43 |
| 🆕 🧠 🧪 [HumanCompatibleAI/tensor-trust](https://github.com/HumanCompatibleAI/tensor-trust) | A prompt injection game to collect data for robust ML research | 🧬 Model / ML Security | 78 | +0 / +0 | `Python` | 42 |
| 🆕 🧠 🧪 [Justinuse1/pojia-breaker](https://github.com/Justinuse1/pojia-breaker) | Chinese-first LLM jailbreak benchmark & red-team toolkit \| 中文LLM安全评测框架 \| by POJIA.AI | ⚔️ AI Red Teaming | 10 | +0 / +0 | `JavaScript` | 42 |
| 🆕 🧠 🧪 [EvanThomasLuke/Awesome-AI-Security-Benchmarks](https://github.com/EvanThomasLuke/Awesome-AI-Security-Benchmarks) | List of AI Security benchmarks | ⚔️ AI Red Teaming | 35 | +0 / +0 | `Unknown` | 39 |
| 🆕 🧠 🧪 [shlomihod/awesome-ai-red-teaming](https://github.com/shlomihod/awesome-ai-red-teaming) | A curated list of awesome AI Red Teaming resources and tools. | ⚔️ AI Red Teaming | 36 | +0 / +0 | `Unknown` | 38 |
| 🆕 🧪 [dreadnode/AIRTBench-Code](https://github.com/dreadnode/AIRTBench-Code) | Code Repository for: AIRTBench: Measuring Autonomous AI Red Teaming Capabilities in Language Models | ⚔️ AI Red Teaming | 111 | +0 / +0 | `Jupyter Notebook` | 35 |
| 🆕 🧠 🧪 [aira-security/Vulnerable-AI-Chatbot](https://github.com/aira-security/Vulnerable-AI-Chatbot) | An intentionally vulnerable AI chatbot to learn and practice AI Security. | 🧪 Labs | 43 | +0 / +0 | `HTML` | 35 |
| 🆕 🧪 [pasquini-dario/project_mantis](https://github.com/pasquini-dario/project_mantis) | Project Mantis: Hacking Back the AI-Hacker; Prompt Injection as a Defense Against LLM-driven Cyberattacks | 🛡️ LLM Application Security | 123 | +0 / +0 | `Python` | 31 |
| 🆕 🧪 [xaoLiu1222/cc-bos](https://github.com/xaoLiu1222/cc-bos) | CC-BOS: Classical Chinese + Bio-inspired Optimization Search for LLM Jailbreak (arXiv:2602.22983) | 🛡️ LLM Application Security | 42 | +0 / +0 | `Python` | 17 |
| 🆕 🧪 [aivillage/llm_verification](https://github.com/aivillage/llm_verification) | LLM prompt attacks for hacker CTFs via CTFd. | 🛡️ LLM Application Security | 15 | +0 / +0 | `Python` | 15 |

### MCP Labs

| Lab | Description | Category | Stars | Growth | Language | Score |
|---|---|---|---:|---:|---|---:|
| 🆕 💎 🧠 🧪 [ottosulin/awesome-ai-security](https://github.com/ottosulin/awesome-ai-security) | A collection of awesome resources related AI security | 🧩 MCP Security | 1,498 | +0 / +0 | `Unknown` | 101 |
| 🆕 💎 🧠 🧪 [sonuoffsec/AI-Security-Hub](https://github.com/sonuoffsec/AI-Security-Hub) | A comprehensive AI Security Hub featuring payloads, cheat sheets, hands-on labs, security tools, MCP security, RAG se... | 🧩 MCP Security | 18 | +0 / +0 | `Unknown` | 98 |
| 🆕 💎 🧠 🧪 [scadastrangelove/awesome-ai-security-tools](https://github.com/scadastrangelove/awesome-ai-security-tools) | A curated list of public-source, research, and commercial tools for AI security and AI-assisted cybersecurity — autot... | 🧩 MCP Security | 1,546 | +0 / +0 | `Python` | 97 |
| 🆕 💎 🧠 🧪 [brinhosa/awesome-ai-security](https://github.com/brinhosa/awesome-ai-security) | A collection of awesome AI Security, LLM Security, and Prompt Injection tools and resources. | 🧩 MCP Security | 37 | +0 / +0 | `Unknown` | 90 |
| 🆕 💎 🧠 🧪 [ProjectRecon/awesome-ai-agents-security](https://github.com/ProjectRecon/awesome-ai-agents-security) | A living map of the AI agent security ecosystem. | 🧩 MCP Security | 71 | +0 / +0 | `Unknown` | 86 |
| 🆕 💎 🧠 🧪 [anmolksachan/AI-ML-Free-Resources-for-Security-and-Prompt-Injection](https://github.com/anmolksachan/AI-ML-Free-Resources-for-Security-and-Prompt-Injection) | AI/ML Pentesting Roadmap for Beginners | 🧩 MCP Security | 794 | +0 / +0 | `Unknown` | 84 |
| 🆕 💎 🧠 🧪 [DmitrL-dev/AISecurity](https://github.com/DmitrL-dev/AISecurity) | Open-source AI security lab: EN/RU academy, public Rust defenses, research and local input evaluation. Explore Specto... | 🧩 MCP Security | 111 | +0 / +0 | `Python` | 82 |
| 🆕 💎 🧠 🧪 [sonuoffsec/DVAP](https://github.com/sonuoffsec/DVAP) | An OWASP-aligned intentionally vulnerable platform for learning and testing AI, LLM, RAG, MCP, and Agentic AI security. | 🧩 MCP Security | 29 | +0 / +0 | `TypeScript` | 75 |
| 🆕 💎 🧠 🧪 [gmh5225/awesome-ai-security](https://github.com/gmh5225/awesome-ai-security) | A curated list of AI Security materials and resources for Pentesters, Bug Hunters, and Security Researchers. | 🧩 MCP Security | 50 | +0 / +0 | `Unknown` | 74 |
| 🆕 💎 🧠 🧪 [fevziegeyurtsevenler/awesome-ai-security-tr](https://github.com/fevziegeyurtsevenler/awesome-ai-security-tr) | Türkçe yapay zeka güvenliği kaynakları — prompt injection, jailbreak, red teaming, guardrail, ajan/MCP ve RAG güvenli... | 🧩 MCP Security | 13 | +0 / +0 | `Python` | 73 |
| 🆕 💎 🧠 🧪 [Joe-B-Security/awesome-prompt-injection](https://github.com/Joe-B-Security/awesome-prompt-injection) | Learn about a type of vulnerability that specifically targets machine learning models | 🧩 MCP Security | 656 | +0 / +0 | `Unknown` | 72 |
| 🆕 💎 🧠 🧪 [Yeti-791/Awesome-Offensive-AI-Agentic-Landscape](https://github.com/Yeti-791/Awesome-Offensive-AI-Agentic-Landscape) | This document curates open-source projects, academic papers, capability benchmarks, and commercial solutions (interna... | 🧩 MCP Security | 310 | +0 / +0 | `Unknown` | 65 |
| 🆕 💎 🧠 🧪 [Arcanum-Sec/ai-sec-resources](https://github.com/Arcanum-Sec/ai-sec-resources) | AI Security Resources Hub | 🧩 MCP Security | 506 | +0 / +0 | `HTML` | 62 |
| 🆕 💎 🧠 🧪 [Threekiii/Awesome-AI-Redteam](https://github.com/Threekiii/Awesome-AI-Redteam) | 一个 AI 攻防知识库。A knowledge base for AI security. | 🧩 MCP Security | 51 | +0 / +0 | `Unknown` | 60 |
| 🆕 💎 🧠 🧪 [hi-unc1e/Auto_JB_APE](https://github.com/hi-unc1e/Auto_JB_APE) | 引擎就一条回路：侦察 → 规划 → 提交 → 判定 → 变异。纯标准库，零框架依赖。内置裁判打 S/A/B/C，canary 或正则命中才算赢，没坐实的不提交。334 个测试离线全绿。CLI、Python API、MCP。One lo... | 🧩 MCP Security | 13 | +0 / +0 | `Python` | 52 |
| 🆕 🧠 🧪 [EvanThomasLuke/Awesome-AI-Security-Skills](https://github.com/EvanThomasLuke/Awesome-AI-Security-Skills) | A list of AI security skills | 🧩 MCP Security | 55 | +0 / +0 | `Unknown` | 49 |
| 🆕 🧠 🧪 [mcp-security-project/awesome-agentic-mcp-security](https://github.com/mcp-security-project/awesome-agentic-mcp-security) | Awesome Agentic MCP Security List | 🧩 MCP Security | 21 | +0 / +0 | `Unknown` | 48 |
| 🆕 🧠 🧪 [ZeroDayEvil/ai-security-tool](https://github.com/ZeroDayEvil/ai-security-tool) | 🛡️ Free open-source AI-powered security terminal & vulnerability scanner (CVE, SBOM). Supports SSH, SFTP, RDP, VNC, S... | 🧩 MCP Security | 414 | +0 / +0 | `HTML` | 45 |
| 🆕 🧠 🧪 [smartdevil09/AI-Security-Professional-Roadmap](https://github.com/smartdevil09/AI-Security-Professional-Roadmap) | No description. | 🧩 MCP Security | 33 | +0 / +0 | `Unknown` | 40 |
| 🆕 🧠 🧪 [dwarvesf/claude-guardrails](https://github.com/dwarvesf/claude-guardrails) | Hardened security configuration for Claude Code; permission deny rules, shell hooks, and prompt injection defense in ... | 🧩 MCP Security | 36 | +0 / +0 | `Shell` | 38 |
| 🆕 🧪 [PawelKozy/mcp-breach-to-fix-labs](https://github.com/PawelKozy/mcp-breach-to-fix-labs) | Hands-on MCP security lab: 10 real incidents reproduced with vulnerable/secure MCP servers, pytest regressions, and C... | 🧩 MCP Security | 89 | +0 / +0 | `Python` | 22 |

### ML Labs

| Lab | Description | Category | Stars | Growth | Language | Score |
|---|---|---|---:|---:|---|---:|
| 🆕 🧪 [orcasecurity-research/AIGoat](https://github.com/orcasecurity-research/AIGoat) | AIGoat: A deliberately Vulnerable AI Infrastructure. Learn AI security through solving our challenges. | 🧬 Model / ML Security | 290 | +0 / +0 | `CSS` | 36 |

---

## 💎 Highest Quality Signals

| Repo | Category | Type | Trust | Quality | Relevance | Stars |
|---|---|---|---|---:|---:|---:|
| [RiccardoBiosas/awesome-MLSecOps](https://github.com/RiccardoBiosas/awesome-MLSecOps) | 🧩 MCP Security | `Awesome List` | `Community` | 120 | 104 | 473 |
| [Tencent/AI-Infra-Guard](https://github.com/Tencent/AI-Infra-Guard) | 🧩 MCP Security | `Awesome List` | `Trusted Source` | 112 | 62 | 6,593 |
| [ottosulin/awesome-ai-security](https://github.com/ottosulin/awesome-ai-security) | 🧩 MCP Security | `Lab` | `Community` | 101 | 69 | 1,498 |
| [sonuoffsec/AI-Security-Hub](https://github.com/sonuoffsec/AI-Security-Hub) | 🧩 MCP Security | `Lab` | `Community` | 98 | 88 | 18 |
| [requie/LLMSecurityGuide](https://github.com/requie/LLMSecurityGuide) | 🧩 MCP Security | `Research / Benchmark` | `Community` | 98 | 87 | 152 |
| [scadastrangelove/awesome-ai-security-tools](https://github.com/scadastrangelove/awesome-ai-security-tools) | 🧩 MCP Security | `Lab` | `Community` | 97 | 65 | 1,546 |
| [wearetyomsmnv/Awesome-LLMSecOps](https://github.com/wearetyomsmnv/Awesome-LLMSecOps) | 🧩 MCP Security | `Awesome List` | `Community` | 96 | 84 | 161 |
| [jassics/awesome-genai-security](https://github.com/jassics/awesome-genai-security) | 🧩 MCP Security | `Awesome List` | `Community` | 93 | 83 | 77 |
| [AISecurityConsortium/AIGoat](https://github.com/AISecurityConsortium/AIGoat) | ⚔️ AI Red Teaming | `Lab` | `Community` | 92 | 82 | 93 |
| [brinhosa/awesome-ai-security](https://github.com/brinhosa/awesome-ai-security) | 🧩 MCP Security | `Lab` | `Community` | 90 | 80 | 37 |

---

## 🏆 Top Repositories

| Repo | Category | Type | Stars | Forks | Language | License | Score |
|---|---|---|---:|---:|---|---|---:|
| [The-Art-of-Hacking/h4cker](https://github.com/The-Art-of-Hacking/h4cker) | ⚖️ AI Governance / Risk | `Awesome List` | 29,537 | 5,384 | `Jupyter Notebook` | `MIT` | 59 |
| [mksglu/context-mode](https://github.com/mksglu/context-mode) | 🧩 MCP Security | `Research / Benchmark` | 24,078 | 1,738 | `TypeScript` | `NOASSERTION` | 51 |
| [NVIDIA/SkillSpector](https://github.com/NVIDIA/SkillSpector) | 🧩 MCP Security | `Research / Benchmark` | 18,336 | 1,591 | `Python` | `Apache-2.0` | 87 |
| [Portkey-AI/gateway](https://github.com/Portkey-AI/gateway) | 🧩 MCP Security | `Tool / Framework` | 13,084 | 1,315 | `TypeScript` | `MIT` | 60 |
| [LouisShark/chatgpt_system_prompt](https://github.com/LouisShark/chatgpt_system_prompt) | 🛡️ LLM Application Security | `Guide / Standard` | 10,778 | 1,460 | `HTML` | `MIT` | 51 |
| [NVIDIA/garak](https://github.com/NVIDIA/garak) | ⚔️ AI Red Teaming | `Research / Benchmark` | 9,361 | 1,309 | `Python` | `Apache-2.0` | 87 |
| [ai-boost/awesome-prompts](https://github.com/ai-boost/awesome-prompts) | 🧩 MCP Security | `Awesome List` | 8,941 | 871 | `Unknown` | `GPL-3.0` | 56 |
| [anthropics/defending-code-reference-harness](https://github.com/anthropics/defending-code-reference-harness) | 🤖 Agent Security | `Tool / Framework` | 7,526 | 622 | `Python` | `NOASSERTION` | 52 |
| [superagent-ai/superagent](https://github.com/superagent-ai/superagent) | 🧩 MCP Security | `Tool / Framework` | 6,754 | 964 | `TypeScript` | `MIT` | 73 |
| [Tencent/AI-Infra-Guard](https://github.com/Tencent/AI-Infra-Guard) | 🧩 MCP Security | `Awesome List` | 6,593 | 617 | `Python` | `Apache-2.0` | 112 |

---

## 📦 Category Summary

| Category | Repositories | Labs | Stars | 7d Growth | Top Repository |
|---|---:|---:|---:|---:|---|
| [⚖️ AI Governance / Risk](#ai-governance-risk) | 10 | 0 | 30,173 | +0 | [The-Art-of-Hacking/h4cker](https://github.com/The-Art-of-Hacking/h4cker) |
| [⚔️ AI Red Teaming](#ai-red-teaming) | 51 | 10 | 14,882 | +0 | [NVIDIA/garak](https://github.com/NVIDIA/garak) |
| [📦 AI Supply Chain Security](#ai-supply-chain-security) | 1 | 0 | 87 | +0 | [Sydsec/syd](https://github.com/Sydsec/syd) |
| [🤖 Agent Security](#agent-security) | 86 | 2 | 24,135 | +0 | [anthropics/defending-code-reference-harness](https://github.com/anthropics/defending-code-reference-harness) |
| [🛡️ LLM Application Security](#llm-application-security) | 99 | 8 | 23,618 | +0 | [LouisShark/chatgpt_system_prompt](https://github.com/LouisShark/chatgpt_system_prompt) |
| [🧪 Labs](#labs) | 2 | 2 | 82 | +0 | [aira-security/Vulnerable-AI-Chatbot](https://github.com/aira-security/Vulnerable-AI-Chatbot) |
| [🧩 MCP Security](#mcp-security) | 181 | 21 | 109,191 | +0 | [mksglu/context-mode](https://github.com/mksglu/context-mode) |
| [🧬 Model / ML Security](#model-ml-security) | 58 | 12 | 21,085 | +0 | [Trusted-AI/adversarial-robustness-toolbox](https://github.com/Trusted-AI/adversarial-robustness-toolbox) |
| [🔐 Other AI Security](#other-ai-security) | 6 | 0 | 2,258 | +0 | [OWASP/www-project-top-10-for-large-language-model-applications](https://github.com/OWASP/www-project-top-10-for-large-language-model-applications) |
| [📚 RAG Security](#rag-security) | 12 | 3 | 895 | +0 | [deadbits/vigil-llm](https://github.com/deadbits/vigil-llm) |

---

## 🗂️ Repository Categories

<a id="ai-governance-risk"></a>

## ⚖️ AI Governance / Risk

**Repositories:** 10  
**Labs:** 0  
**Stars:** 30,173  
**7-day growth:** +0

### Dashboard / Platform

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [ZeroDayEvil/CVE-2026-54121-Certighost](https://github.com/ZeroDayEvil/CVE-2026-54121-Certighost) | 🛡️ Official AI Security Tool module for CVE-2026-54121 (Certighost - AD CS Domain Controller Impersonation & PKINIT E... | 83 | +0 / +0 | `Tool / Framework` | `Unknown` | `Unknown` | 29 |
| 🆕 [ZeroDayEvil/CVE-2026-41096-PoC](https://github.com/ZeroDayEvil/CVE-2026-41096-PoC) | 🛡️ Official AI Security Tool module for CVE-2026-41096 (Windows DNSAPI.dll Heap Overflow / DNS-Mayhem Deep Dive Analy... | 92 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 27 |
| 🆕 [ZeroDayEvil/CVE-2026-20805-PoC](https://github.com/ZeroDayEvil/CVE-2026-20805-PoC) | 🛡️ Official AI Security Tool module for CVE-2026-20805 (Desktop Window Manager / dwm.exe Information Disclosure & Mem... | 86 | +0 / +0 | `Tool / Framework` | `Unknown` | `Unknown` | 26 |
| 🆕 [laboratoriohacker-com/tatu](https://github.com/laboratoriohacker-com/tatu) | Free, open source, self-hosted. Built by Laboratório Hacker for teams that take AI security seriously. Deploy in minu... | 36 | +0 / +0 | `Tool / Framework` | `Python` | `Unknown` | 21 |

### General

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [jiangnanboy/llm_security](https://github.com/jiangnanboy/llm_security) | 利用分类法和敏感词检测法对生成式大模型的输入和输出内容进行安全检测，尽早识别风险内容。The input and output contents of generative large model are checked by cla... | 28 | +0 / +0 | `Project` | `Java` | `MIT` | 12 |

### Guardrails

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [jeanmalaquias/ai-governance-mapping](https://github.com/jeanmalaquias/ai-governance-mapping) | Unified AI governance controls catalog crosswalking NIST AI RMF 1.1, EU AI Act, OWASP LLM Top 10, and ISO/IEC 42001 -... | 13 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 16 |

### RAG Poisoning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [socprime/AIDR-Bastion](https://github.com/socprime/AIDR-Bastion) | A comprehensive GenAI protection system designed to protect against malicious prompts, injection attacks, and harmful... | 106 | +0 / +0 | `Tool / Framework` | `Python` | `LGPL-3.0` | 28 |

### Sandboxing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [ZeroDayEvil/CVE-2026-21858-n8n-FullChain](https://github.com/ZeroDayEvil/CVE-2026-21858-n8n-FullChain) | 🛡️ Official AI Security Tool module for CVE-2026-21858 + CVE-2025-68613 (n8n "Ni8mare" Unauthenticated Arbitrary File... | 86 | +0 / +0 | `Tool / Framework` | `Unknown` | `Unknown` | 26 |

### Tool / Function Calling

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🏆 💎 [The-Art-of-Hacking/h4cker](https://github.com/The-Art-of-Hacking/h4cker) | This repository is maintained by Omar Santos (@santosomar) and includes thousands of resources related to ethical hac... | 29,537 | +0 / +0 | `Awesome List` | `Jupyter Notebook` | `MIT` | 59 |
| 🆕 [Elite588/Hacker](https://github.com/Elite588/Hacker) | This repository is maintained by Hex Core and includes thousands of resources related to ethical hacking, bug bountie... | 106 | +0 / +0 | `Awesome List` | `Jupyter Notebook` | `MIT` | 31 |

[Back to top](#ai-security-repository-radar)

---

<a id="ai-red-teaming"></a>

## ⚔️ AI Red Teaming

**Repositories:** 51  
**Labs:** 10  
**Stars:** 14,882  
**7-day growth:** +0

### Benchmarks / Eval

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [TrustAI-laboratory/LMAP](https://github.com/TrustAI-laboratory/LMAP) | LMAP (large language model mapper) is like NMAP for LLM, is an LLM Vulnerability Scanner and Zero-day Vulnerability F... | 31 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Apache-2.0` | 71 |
| 🆕 💎 🧠 [KazKozDev/system-prompt-benchmark](https://github.com/KazKozDev/system-prompt-benchmark) | Automated Red-Teaming tool for LLM Security. Evaluate system prompts against prompt injection, jailbreak, and data ex... | 17 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 53 |
| 🆕 🧠 [dongdongunique/EvoSynth](https://github.com/dongdongunique/EvoSynth) | EvoSynth is a SOTA automated LLM red-teaming framework that evolves executable, code-level jailbreak attack methods u... | 60 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 45 |
| 🆕 🧠 [WhileBug/AwesomeLLMJailBreakPapers](https://github.com/WhileBug/AwesomeLLMJailBreakPapers) | Awesome LLM Jailbreak academic papers | 174 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 42 |
| 🆕 🧠 [weidutech/pi-recon](https://github.com/weidutech/pi-recon) | Lightweight AI red teaming harness for authorized agent / GenAI challenges (BCTF Agent Plus–style): schedule, bounded... | 26 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 38 |
| 🆕 🧠 [NaviRocker/llm-red-teaming-dataset](https://github.com/NaviRocker/llm-red-teaming-dataset) | A comprehensive dataset of red teaming prompts designed to evaluate and improve the safety mechanisms of LLMs | 12 | +0 / +0 | `Research / Benchmark` | `Unknown` | `MIT` | 36 |
| 🆕 [GenggengSvan/Jailbreak-Observatory](https://github.com/GenggengSvan/Jailbreak-Observatory) | LLM Jailbreak Research Trend Analysis Repository | 23 | +0 / +0 | `Awesome List` | `Python` | `Unknown` | 25 |
| 🆕 [xsankar/AI-Red-Teaming](https://github.com/xsankar/AI-Red-Teaming) | All things specific to LLM Red Teaming Generative AI | 30 | +0 / +0 | `Awesome List` | `Unknown` | `MIT` | 22 |

### Dashboard / Platform

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🏆 💎 🧠 ✅ [NVIDIA/garak](https://github.com/NVIDIA/garak) | the LLM vulnerability scanner | 9,361 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 87 |
| 🆕 💎 🧠 [Arnoldlarry15/ARES-Dashboard](https://github.com/Arnoldlarry15/ARES-Dashboard) | AI Red Team Operations Console | 16 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 53 |
| 🆕 [humane-intelligence/hi-oss-ai-red-teaming](https://github.com/humane-intelligence/hi-oss-ai-red-teaming) | A platform for running human-centered red teaming evaluations on AI models and systems. | 12 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 32 |

### General

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [joey-melo/payloads](https://github.com/joey-melo/payloads) | Payloads for AI Red Teaming and beyond | 324 | +0 / +0 | `Project` | `Unknown` | `MIT` | 21 |
| 🆕 [TakSec/AI-Red-Teaming-Everything](https://github.com/TakSec/AI-Red-Teaming-Everything) | Welcome to the AI Red Teaming Everything repository! This repository aims to provide techniques for red teaming AI sy... | 10 | +0 / +0 | `Project` | `Unknown` | `MIT` | 18 |
| 🆕 [db0109/AI-Red-Team-Scripts-And-Checklist](https://github.com/db0109/AI-Red-Team-Scripts-And-Checklist) | Scripts and cheat sheet I use for AI red teaming, thanks to Digital Andrew for many of the scripts and the foundation... | 14 | +0 / +0 | `Guide / Standard` | `Python` | `Unknown` | 13 |

### Guardrails

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 ✅ [tldrsec/prompt-injection-defenses](https://github.com/tldrsec/prompt-injection-defenses) | Every practical and proposed defense against prompt injection. | 734 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 62 |
| 🆕 💎 🧠 [ReversecLabs/spikee](https://github.com/ReversecLabs/spikee) | Simple Prompt Injection Kit for Evaluation and Exploitation | 258 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 58 |
| 🆕 💎 🧠 [pazo01/awesome-cyber-ai-arsenal](https://github.com/pazo01/awesome-cyber-ai-arsenal) | A curated collection of offensive, defensive and AI/LLM security tools. | 173 | +0 / +0 | `Awesome List` | `Unknown` | `MIT` | 51 |
| 🆕 🧠 ✅ [randalltr/universal-llm-jailbreak-hiddenlayer](https://github.com/randalltr/universal-llm-jailbreak-hiddenlayer) | A universal LLM jailbreak using HiddenLayer’s Policy Puppetry attack — tested on GPT-4, Claude, Gemini, and open-sour... | 24 | +0 / +0 | `Tool / Framework` | `Unknown` | `Unknown` | 49 |
| 🆕 🧠 [enguard-ai/awesome-ai-guardrails](https://github.com/enguard-ai/awesome-ai-guardrails) | A curated list of materials on AI guardrails | 66 | +0 / +0 | `Awesome List` | `Python` | `Apache-2.0` | 48 |
| 🆕 🧠 [SantanderAI/autoguardrails](https://github.com/SantanderAI/autoguardrails) | Alignment-research scaffold (autoresearch-style) for LLM guardrails: search over a single policy.md surface | 130 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 48 |
| 🆕 [LumosLab-Innovation/OpenHunterAI](https://github.com/LumosLab-Innovation/OpenHunterAI) | Local-first AI red team for web, API, and LLM application security. Attacker-style reasoning, evidence-backed finding... | 327 | +0 / +0 | `Tool / Framework` | `TypeScript` | `NOASSERTION` | 35 |
| 🆕 [cybertechajju/LLM-PROMPT-INJECTION-PAYLOAD-S](https://github.com/cybertechajju/LLM-PROMPT-INJECTION-PAYLOAD-S) | Unlock safe, high-signal prompt workflows for ethical hacking and AI red-teaming | 25 | +0 / +0 | `Tool / Framework` | `Unknown` | `Unknown` | 31 |
| 🆕 [TUD-ARTS-2023/LLM-red-teaming-prompts](https://github.com/TUD-ARTS-2023/LLM-red-teaming-prompts) | LLM red teaming datasets from the paper 'Student-Teacher Prompting for Red Teaming to Improve Guardrails' for the ART... | 26 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 15 |

### Jailbreak Testing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧠 [ahow2004/injectlab](https://github.com/ahow2004/injectlab) | An open-source ATT&CK-style framework and test suite for adversarial LLM security research | 10 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 44 |
| 🆕 🧠 [erfnzdeh/MCI-LLM-Security-Hackathon](https://github.com/erfnzdeh/MCI-LLM-Security-Hackathon) | Contest diary of team Prompt Police at MCI's LLM-security hackathon (Quera, Nov 2025): jailbreaking safety-guarded mo... | 100 | +0 / +0 | `Tool / Framework` | `Jupyter Notebook` | `Unknown` | 43 |
| 🆕 🧠 [yunwei37/prompt-hacker-collections](https://github.com/yunwei37/prompt-hacker-collections) | prompt attack-defense, prompt Injection, reverse engineering notes and examples \| 提示词对抗、破解例子与笔记 | 365 | +0 / +0 | `Awesome List` | `Unknown` | `MIT` | 42 |
| 🆕 🧠 [BlueprintLabIO/prompt-injector](https://github.com/BlueprintLabIO/prompt-injector) | A minimal TypeScript library with research informed prompt injection attacks. | 52 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `Unknown` | 40 |
| 🆕 🧠 [xirui-li/DrAttack](https://github.com/xirui-li/DrAttack) | Official implementation of paper: DrAttack: Prompt Decomposition and Reconstruction Makes Powerful LLM Jailbreakers | 68 | +0 / +0 | `Research / Benchmark` | `JavaScript` | `MIT` | 39 |
| 🆕 🧠 [buryusu/All-AI-Jailbreaks](https://github.com/buryusu/All-AI-Jailbreaks) | Curated archive of jailbreak and prompt-injection experiments for AI model robustness research, red-teaming, and defe... | 57 | +0 / +0 | `Research / Benchmark` | `Batchfile` | `Unknown` | 35 |
| 🆕 [volt-l18/llm-redteam-pipeline](https://github.com/volt-l18/llm-redteam-pipeline) | An automated, multi-turn LLM red-teaming pipeline that pits an Attacker model against a Target model to evaluate safe... | 21 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 33 |
| 🆕 [m4xx101/cryptex-oss](https://github.com/m4xx101/cryptex-oss) | Open-source LLM red-teaming technique toolkit (162 transforms, 36 mutators, 25 tool surfaces). MIT. | 336 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 28 |
| 🆕 [TenXxs/AI-Security-for-IoT-and-Mobile](https://github.com/TenXxs/AI-Security-for-IoT-and-Mobile) | No description. | 49 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 17 |

### LLM Labs

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 🧪 [AISecurityConsortium/AIGoat](https://github.com/AISecurityConsortium/AIGoat) | AIGoat - Open-source AI security playground for LLM red teaming. AI Goat provides hands-on labs covering the full OWA... | 93 | +0 / +0 | `Lab` | `JavaScript` | `Apache-2.0` | 92 |
| 🆕 💎 🧠 🧪 [Masriyan/Claude-Code-CyberSecurity-Skill](https://github.com/Masriyan/Claude-Code-CyberSecurity-Skill) | 22 production-quality Claude Code Skills for cybersecurity professionals — covering offensive security, defensive ope... | 446 | +0 / +0 | `Lab` | `Python` | `MIT` | 64 |
| 🆕 💎 🧠 🧪 [user1342/Awesome-LLM-Red-Teaming](https://github.com/user1342/Awesome-LLM-Red-Teaming) | A curated list of awesome LLM Red Teaming training, resources, and tools. | 135 | +0 / +0 | `Lab` | `Unknown` | `MIT` | 62 |
| 🆕 🧠 🧪 [gatsby-sec/llm-sec-range](https://github.com/gatsby-sec/llm-sec-range) | 🛡️ 大模型攻防渗透测试靶场 · 提示注入CTF / OWASP LLM Top10 / 脆弱Agent / 资料聚合。目标模型可切换：DeepSeek直连、OpenRouter中转站(国产+国外十余款小模型)、本地Ollama(De... | 30 | +0 / +0 | `Lab` | `Python` | `MIT` | 46 |
| 🆕 🧠 🧪 [R3dShad0w7/PromptMe](https://github.com/R3dShad0w7/PromptMe) | PromptMe is an educational project that showcases security vulnerabilities in large language models (LLMs) and their ... | 111 | +0 / +0 | `Lab` | `Python` | `Apache-2.0` | 46 |
| 🆕 🧠 🧪 [randalltr/prompt-engineering-for-hackers](https://github.com/randalltr/prompt-engineering-for-hackers) | Prompt Engineering for Hackers: A Hands-On Intro to LLMs, Jailbreaks, and Adversarial Prompting | 11 | +0 / +0 | `Lab` | `Unknown` | `NOASSERTION` | 43 |
| 🆕 🧠 🧪 [Justinuse1/pojia-breaker](https://github.com/Justinuse1/pojia-breaker) | Chinese-first LLM jailbreak benchmark & red-team toolkit \| 中文LLM安全评测框架 \| by POJIA.AI | 10 | +0 / +0 | `Lab` | `JavaScript` | `MIT` | 42 |
| 🆕 🧠 🧪 [EvanThomasLuke/Awesome-AI-Security-Benchmarks](https://github.com/EvanThomasLuke/Awesome-AI-Security-Benchmarks) | List of AI Security benchmarks | 35 | +0 / +0 | `Lab` | `Unknown` | `Unknown` | 39 |
| 🆕 🧠 🧪 [shlomihod/awesome-ai-red-teaming](https://github.com/shlomihod/awesome-ai-red-teaming) | A curated list of awesome AI Red Teaming resources and tools. | 36 | +0 / +0 | `Lab` | `Unknown` | `Unknown` | 38 |
| 🆕 🧪 [dreadnode/AIRTBench-Code](https://github.com/dreadnode/AIRTBench-Code) | Code Repository for: AIRTBench: Measuring Autonomous AI Red Teaming Capabilities in Language Models | 111 | +0 / +0 | `Lab` | `Jupyter Notebook` | `Apache-2.0` | 35 |

### Prompt Injection Defense

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [GURPREETKAURJETHRA/LLM-SECURITY](https://github.com/GURPREETKAURJETHRA/LLM-SECURITY) | Securing LLM's Against Top 10 OWASP Large Language Model Vulnerabilities 2024 | 23 | +0 / +0 | `Research / Benchmark` | `Unknown` | `MIT` | 64 |
| 🆕 [Arcanum-Sec/arc_pi_taxonomy](https://github.com/Arcanum-Sec/arc_pi_taxonomy) | The Arcanum Prompt Injection Taxonomy | 763 | +0 / +0 | `Tool / Framework` | `Unknown` | `NOASSERTION` | 37 |
| 🆕 🧠 [rushout09/llm-security-startups](https://github.com/rushout09/llm-security-startups) | An awesome and comprehensive list of LLM Securtiy Startups. | 15 | +0 / +0 | `Awesome List` | `Unknown` | `GPL-3.0` | 35 |
| 🆕 🧠 [peluche/deck-of-many-prompts](https://github.com/peluche/deck-of-many-prompts) | Manual Prompt Injection / Red Teaming Tool | 59 | +0 / +0 | `Project` | `Python` | `Unknown` | 35 |
| 🆕 🧠 [AdityaBhatt3010/Hacking-Lakera-Gandalf-AI-via-Prompt-Injection](https://github.com/AdityaBhatt3010/Hacking-Lakera-Gandalf-AI-via-Prompt-Injection) | Lakera Gandalf AI challenge's step by step walkthrough, showcasing real-world prompt injection techniques and LLM sec... | 15 | +0 / +0 | `Project` | `Unknown` | `MIT` | 32 |
| 🆕 [mavhezha/htb-ai-red-teamer](https://github.com/mavhezha/htb-ai-red-teamer) | No description. | 13 | +0 / +0 | `Project` | `Python` | `Unknown` | 26 |

### RAG Poisoning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧠 [detoxio-ai/ai-red-teaming-training](https://github.com/detoxio-ai/ai-red-teaming-training) | No description. | 24 | +0 / +0 | `Guide / Standard` | `HTML` | `MIT` | 30 |
| 🆕 [JosephTLucas/airt_utils](https://github.com/JosephTLucas/airt_utils) | Utilities for experiment-centric AI Red Teaming workflows | 11 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 17 |

### Tool / Function Calling

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [antojoseph/adv26](https://github.com/antojoseph/adv26) | This repo will be updated with the AI red team lab material for Adveserial village at Defcon 2026 | 10 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 22 |

[Back to top](#ai-security-repository-radar)

---

<a id="ai-supply-chain-security"></a>

## 📦 AI Supply Chain Security

**Repositories:** 1  
**Labs:** 0  
**Stars:** 87  
**7-day growth:** +0

### RAG Poisoning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [Sydsec/syd](https://github.com/Sydsec/syd) | Offline AI Security Assistant for Air-Gapped Pentesting | 87 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 18 |

[Back to top](#ai-security-repository-radar)

---

<a id="agent-security"></a>

## 🤖 Agent Security

**Repositories:** 86  
**Labs:** 2  
**Stars:** 24,135  
**7-day growth:** +0

### Benchmarks / Eval

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [0xSweet/awesome-llm-security-alignment](https://github.com/0xSweet/awesome-llm-security-alignment) | A curated list of research papers, experiments, and resources related to LLM security and alignment. | 12 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 81 |
| 🆕 💎 🧠 ✅ [meta-llama/PurpleLlama](https://github.com/meta-llama/PurpleLlama) | Set of tools to assess and improve LLM security. | 4,406 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 80 |
| 🆕 💎 [SharpAI/DeepCamera](https://github.com/SharpAI/DeepCamera) | Open-Source AI Camera Skills Platform, AI NVR & CCTV Surveillance. Local VLM video analysis with Qwen, DeepSeek, Smol... | 3,074 | +0 / +0 | `Research / Benchmark` | `JavaScript` | `MIT` | 59 |
| 🆕 🧠 [NY1024/AgentSafety-Papers](https://github.com/NY1024/AgentSafety-Papers) | Daily Tracking of LLM Agent Security Papers on arXiv | 11 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 47 |
| 🆕 🧠 [sleeepeer/PIArena](https://github.com/sleeepeer/PIArena) | [ACL 2026] PIArena: A Platform for Prompt Injection Evaluation | 52 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 37 |
| 🆕 [CiscoDevNet/foundry-security-spec](https://github.com/CiscoDevNet/foundry-security-spec) | An open specification for agentic AI security evaluation and testing, from Cisco. | 223 | +0 / +0 | `Research / Benchmark` | `Unknown` | `NOASSERTION` | 36 |
| 🆕 [agiresearch/ASB](https://github.com/agiresearch/ASB) | Agent Security Bench (ASB) | 305 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 34 |
| 🆕 [jiaxiaojunQAQ/SkillJect](https://github.com/jiaxiaojunQAQ/SkillJect) | SkillJect: Automating Stealthy Skill-Based Prompt Injection for Coding Agents with Trace-Driven Closed-Loop Refinement | 79 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 29 |
| 🆕 [albert-y1n/PISmith](https://github.com/albert-y1n/PISmith) | PISmith: Reinforcement Learning-based Red Teaming for Prompt Injection Defenses(COLM 2026) | 31 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 24 |
| 🆕 [SaFo-Lab/AgentDyn](https://github.com/SaFo-Lab/AgentDyn) | The official implementation of the paper "AgentDyn: Are Your Agent Security Defenses Deployable in Real-World Dynamic... | 82 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 22 |
| 🆕 [lihouwenbin/ai-redteam-recursive-self-improvement](https://github.com/lihouwenbin/ai-redteam-recursive-self-improvement) | Domain-neutral AI red-team framework for recursive self-improvement governance | 35 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 21 |
| 🆕 [henrymao2004/Auto-research-red-teaming](https://github.com/henrymao2004/Auto-research-red-teaming) | AHA is autoresearch for agent safety: one agent red-teams a production agent overnight and comes back with reusable e... | 29 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 21 |
| 🆕 [Yunhao-Feng/AgentBackdoorBench](https://github.com/Yunhao-Feng/AgentBackdoorBench) | Paper of AI Agent Security | 10 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 20 |

### Dashboard / Platform

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 ✅ [SleuthCo/clawshield-public](https://github.com/SleuthCo/clawshield-public) | Security proxy for AI agents. Scans every message for prompt injection, PII, and secrets. Defense-in-depth: Go proxy ... | 130 | +0 / +0 | `Research / Benchmark` | `Go` | `Apache-2.0` | 61 |
| 🆕 🧠 [akav-labs/agentsentry-gateway](https://github.com/akav-labs/agentsentry-gateway) | Transparent OpenAI-compatible security gateway for LLM agents — DLP, secret-leak, jailbreak & prompt-injection detect... | 10 | +0 / +0 | `Tool / Framework` | `Rust` | `Apache-2.0` | 43 |
| 🆕 🧠 [techlab-innov/llmtrace](https://github.com/techlab-innov/llmtrace) | Zero-code LLM security & observability proxy. Real-time prompt injection detection, PII scanning, and cost control fo... | 53 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 42 |
| 🆕 🧠 [vinceAmstoutz/symfony-security-auditor](https://github.com/vinceAmstoutz/symfony-security-auditor) | AI-powered multi-agent security auditor for Symfony applications — provider-agnostic via symfony/ai. | 95 | +0 / +0 | `Tool / Framework` | `PHP` | `MIT` | 35 |
| 🆕 🧠 [SyntaxMethod/CVE-2026-42978-PoC-Research](https://github.com/SyntaxMethod/CVE-2026-42978-PoC-Research) | CVE-2026-42978 Windows Push Notifications (WpnService) Use-After-Free & Race Condition PoC research, diagnostic scann... | 94 | +0 / +0 | `Tool / Framework` | `Unknown` | `MIT` | 35 |
| 🆕 [MicroAIInc/MicroAI-Security-and-Monitoring](https://github.com/MicroAIInc/MicroAI-Security-and-Monitoring) | No description. | 60 | +0 / +0 | `Guide / Standard` | `Python` | `NOASSERTION` | 28 |
| 🆕 [royapakzad/multilingual-ai-safety-evaluation](https://github.com/royapakzad/multilingual-ai-safety-evaluation) | Evaluate LLM safety and performance across non-English languages. | 13 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `NOASSERTION` | 26 |
| 🆕 [ruvnet/agentic-security](https://github.com/ruvnet/agentic-security) | No description. | 51 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 24 |
| 🆕 [akramIOT/Agentic-IAM](https://github.com/akramIOT/Agentic-IAM) | Agentic Identity & Agentic Security System | 13 | +0 / +0 | `Tool / Framework` | `Python` | `Unknown` | 24 |
| 🆕 [Leviticus-Triage/cerebro-red-v2](https://github.com/Leviticus-Triage/cerebro-red-v2) | CEREBRO-RED v2: Advanced LLM Red Team Research Platform with PAIR Algorithm and LLM-as-a-Judge Evaluation | 16 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 22 |

### Framework / Library

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [secnova-ai/ClawdSecbot](https://github.com/secnova-ai/ClawdSecbot) | ClawdSecbot is a professional AI Bot security protection solution, providing security capabilities including real-tim... | 94 | +0 / +0 | `Tool / Framework` | `Go` | `GPL-3.0` | 28 |
| 🆕 [tegridydev/multi-agent-secops-llm](https://github.com/tegridydev/multi-agent-secops-llm) | This project is a multi-agent security framework that utilizes multiple LLM models to analyze and generate comprehens... | 10 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 26 |
| 🆕 [GenAI-Security-Project/GenAI-Agent-Security-Initiative](https://github.com/GenAI-Security-Project/GenAI-Agent-Security-Initiative) | This repository contains examples of insecure code and/or security misconfigurations in common Agent frameworks | 21 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 21 |

### Guardrails

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [CollieAi/llm-firewall](https://github.com/CollieAi/llm-firewall) | AI Firewall & LLM security toolkit - protect your AI applications from prompt injection, jailbreaks, PII leakage, and... | 43 | +0 / +0 | `Tool / Framework` | `Unknown` | `MIT` | 88 |
| 🆕 💎 🧠 [msoedov/agentic_security](https://github.com/msoedov/agentic_security) | Agentic LLM Vulnerability Scanner / AI red teaming kit 🧪 | 2,007 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 86 |
| 🆕 💎 🧠 [doronp/agentshield-benchmark](https://github.com/doronp/agentshield-benchmark) | Open benchmark for AI agent security tools — prompt injection, data exfiltration, tool abuse, provenance | 27 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `Apache-2.0` | 62 |
| 🆕 🧠 [vstorm-co/pydantic-ai-shields](https://github.com/vstorm-co/pydantic-ai-shields) | Guardrail capabilities for Pydantic AI — cost tracking, prompt injection detection, PII filtering, secret redaction, ... | 94 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 45 |
| 🆕 🧠 [kagnlp/Awesome-Agentic-Security](https://github.com/kagnlp/Awesome-Agentic-Security) | A curated list of 260+ papers and resources on Agentic Security. Based on the survey covering the transition from pas... | 56 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 45 |
| 🆕 🧠 [ethanolivertroy/unpromptedcon-2026-slides](https://github.com/ethanolivertroy/unpromptedcon-2026-slides) | Community collection of all available slide decks from [un]prompted 2026, the AI Security Practitioner Conference (Ma... | 47 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 42 |
| 🆕 🧠 [dcarpintero/pangolin-guard](https://github.com/dcarpintero/pangolin-guard) | Lightweight, Self-Hosted AI Guardrails Model based on ModernBERT. | 12 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `Apache-2.0` | 39 |
| 🆕 🧠 [CloudDefenseAI/secure-agents-md](https://github.com/CloudDefenseAI/secure-agents-md) | Security working agreements for AI coding agents: hardened AGENTS.md, prompt/tool-injection guardrails, dependency hy... | 12 | +0 / +0 | `Research / Benchmark` | `Unknown` | `MIT` | 34 |
| 🆕 [martian56/redcell](https://github.com/martian56/redcell) | AI red-team platform. Autonomous LLM agents run a penetration test end to end inside a Kali container and write the r... | 318 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 33 |
| 🆕 [gryszzz/open-thymos](https://github.com/gryszzz/open-thymos) | AI agent sandbox prison runtime with a local-first desktop app + CLI. Chat with any model (Claude, OpenAI, Groq, Olla... | 18 | +0 / +0 | `Tool / Framework` | `Rust` | `Apache-2.0` | 30 |
| 🆕 [isartor-ai/Isartor](https://github.com/isartor-ai/Isartor) | Open-source Prompt Firewall — deflect up to 95% of redundant LLM traffic before it leaves your infrastructure. Docume... | 21 | +0 / +0 | `Research / Benchmark` | `Rust` | `Apache-2.0` | 26 |
| 🆕 [arkanemystic/lean-agent-protocol](https://github.com/arkanemystic/lean-agent-protocol) | Formalized AI guardrails for financial agents that replace probabilistic filters with Lean 4 mathematical proofs, ens... | 14 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `Apache-2.0` | 26 |
| 🆕 [NIKHIL-KASHMEERABEN-RUPALA/Neural_Forge](https://github.com/NIKHIL-KASHMEERABEN-RUPALA/Neural_Forge) | NeuralForge is an end-to-end LLM fine-tuning & serving platform. It leverages LoRA (PyTorch/Hugging Face) for domain ... | 18 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 26 |

### Jailbreak Testing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [SnailSploit/The-LLM-Red-Teamer-s-Playbook](https://github.com/SnailSploit/The-LLM-Red-Teamer-s-Playbook) | A diagnostic methodology for bypassing LLM defense layers — from input filters to persistent memory exploitation. | 49 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 64 |
| 🆕 💎 🧠 [zakky8/llm-jailbreak-taxonomy](https://github.com/zakky8/llm-jailbreak-taxonomy) | Mechanism-grounded taxonomy of 40 LLM jailbreak patterns across 10 categories. 8,000-trial bootstrap evaluation for t... | 14 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `MIT` | 61 |
| 🆕 💎 🧠 [ZenGuard-AI/fast-llm-security-guardrails](https://github.com/ZenGuard-AI/fast-llm-security-guardrails) | The fastest Trust Layer for AI Agents | 155 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 54 |
| 🆕 🧠 [solanabr/auditor-skill](https://github.com/solanabr/auditor-skill) | Claude Code / agentic security skill for Solana programs and software. Full audit-firm lifecycle, executable PoC + fi... | 55 | +0 / +0 | `Research / Benchmark` | `Rust` | `MIT` | 43 |
| 🆕 [mega-edo/mega-security-leaderboard](https://github.com/mega-edo/mega-security-leaderboard) | Leaderboard Comparing LLM Agent Security on System Prompt Leakage and Attack Probes | 20 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 29 |
| 🆕 [kcsinclair/kaos-control](https://github.com/kcsinclair/kaos-control) | Self-hosted orchestrator for Claude Code agents: drive ideas through a role-gated SDLC (plan → build → test → release... | 16 | +0 / +0 | `Tool / Framework` | `Go` | `NOASSERTION` | 29 |

### LLM Labs

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 🧪 [KryptSec/oasis](https://github.com/KryptSec/oasis) | Open-source AI security benchmarking CLI. Measure how AI models perform offensive security tasks with MITRE ATT&CK an... | 29 | +0 / +0 | `Lab` | `TypeScript` | `MIT` | 52 |
| 🆕 🧠 🧪 [Tcotl/DVLAA](https://github.com/Tcotl/DVLAA) | DVLAA (Damn Vulnerable LLM and Agent Application) is a local LLM and Agent security range for OWASP LLM Top 10 and Ag... | 33 | +0 / +0 | `Lab` | `Python` | `Unknown` | 43 |

### Prompt Injection Defense

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [humanbound/humanbound-firewall](https://github.com/humanbound/humanbound-firewall) | Multi-tier firewall for AI agents — blocks prompt injections, jailbreaks, and scope violations. Local tiers first; LL... | 53 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 57 |
| 🆕 💎 🧠 [occludra/gateway](https://github.com/occludra/gateway) | Self-hosted AI security proxy. Redact PII, block prompt injection, route to any LLM provider. OpenAI-compatible. | 41 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 56 |
| 🆕 🧠 [anishathalye/ai-agent-security-lecture](https://github.com/anishathalye/ai-agent-security-lecture) | Guest lecture in MIT 6.566 on AI Agent Security | 32 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 46 |
| 🆕 🧠 [henio828/unveil](https://github.com/henio828/unveil) | Find text that hides from humans but speaks to machines. Defensive scanner for indirect prompt injection in untrusted... | 10 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 41 |
| 🆕 🧠 [haggaishachar/agentfence](https://github.com/haggaishachar/agentfence) | AgentFence is an open-source platform for automatically testing AI agent security. It identifies vulnerabilities such... | 61 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 40 |
| 🆕 🧠 [antgroup/Agent3Sigma-Canary](https://github.com/antgroup/Agent3Sigma-Canary) | Agent3σ-Canary is an evaluation framework for AI Agent security in realistic runtime environments. | 37 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 37 |
| 🆕 🧠 [Agentic-AI-Risk-Mitigation/Janus](https://github.com/Agentic-AI-Risk-Mitigation/Janus) | System-level security for LLM agents: fine-grained policy enforcement on tool calls to defend against indirect prompt... | 16 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 37 |
| 🆕 🧠 [nevinshine/telos-runtime](https://github.com/nevinshine/telos-runtime) | A closed-loop security runtime preventing "The Great Exfiltration" and Indirect Prompt Injection in Autonomous AI Age... | 18 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 36 |
| 🆕 ✅ [facebookresearch/Meta_SecAlign](https://github.com/facebookresearch/Meta_SecAlign) | Repo for the paper "Meta SecAlign: A Secure Foundation LLM Against Prompt Injection Attacks". | 71 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 36 |
| 🆕 [pors/skill-audit](https://github.com/pors/skill-audit) | Security auditing CLI for AI agent skills - detects prompt injection, secrets, and dangerous code patterns. | 62 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 34 |
| 🆕 [yi-john-huang/openclaw-secure-stack](https://github.com/yi-john-huang/openclaw-secure-stack) | 🔒 One-command secure OpenClaw deployment with built-in skills scanner and prompt injection protection | 56 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 32 |
| 🆕 [kaijiezhu11/MELON](https://github.com/kaijiezhu11/MELON) | [ICML'25] MELON: Provable Defense Against Indirect Prompt Injection Attacks in AI Agents | 39 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 28 |
| 🆕 [zscole/bagman-skill](https://github.com/zscole/bagman-skill) | Secure key management patterns for AI agents - storage, session keys, leak prevention, prompt injection defense | 18 | +0 / +0 | `Research / Benchmark` | `Unknown` | `MIT` | 22 |
| 🆕 [CoworkedShawn/openclaw-skills](https://github.com/CoworkedShawn/openclaw-skills) | Custom OpenClaw skills for AI agent capabilities - memory systems, prompt injection defense, intent routing, and plat... | 12 | +0 / +0 | `Project` | `JavaScript` | `Unknown` | 20 |

### RAG Poisoning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [GenAI-Security-Project/GenAI-Data-Security-Initiative](https://github.com/GenAI-Security-Project/GenAI-Data-Security-Initiative) | OWASP GenAI Data Security Initiative — taxonomies, crosswalks, datasets, and tooling for securing data in generative ... | 26 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 67 |
| 🆕 💎 🧠 [OraclesTech/guardian-sdk](https://github.com/OraclesTech/guardian-sdk) | Ethicore Engine® is an AI safety, ethics, and compliance platform. This repo consists of the open-source components o... | 122 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 61 |
| 🆕 💎 🧠 ✅ [agamm/claude-code-owasp](https://github.com/agamm/claude-code-owasp) | Claude Code skill for OWASP security best practices (2025-2026). Includes Top 10:2025, ASVS 5.0, Agentic AI security,... | 367 | +0 / +0 | `Research / Benchmark` | `JavaScript` | `MIT` | 57 |
| 🆕 💎 🧠 [emmanuelgjr/genai_incidents](https://github.com/emmanuelgjr/genai_incidents) | Single source of truth for GenAI and agentic AI security incidents, mapped to OWASP LLM Top 10, OWASP Agentic Top 10 ... | 29 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 53 |
| 🆕 🧠 [nukIeer/AI-Prompt-Injection-Cheatsheet](https://github.com/nukIeer/AI-Prompt-Injection-Cheatsheet) | AI hacking snippets for prompt injection, jailbreaking LLMs, and bypassing AI filters. Ideal for ethical hackers and ... | 128 | +0 / +0 | `Tool / Framework` | `Unknown` | `Unknown` | 48 |
| 🆕 🧠 [X-Scale-AI/GRITS](https://github.com/X-Scale-AI/GRITS) | Open-source AI agent security framework. Score, harden, and govern your AI agents. Built on DoD/DISA zero-trust princ... | 12 | +0 / +0 | `Research / Benchmark` | `Unknown` | `NOASSERTION` | 46 |
| 🆕 🧠 [anshug/claude-mythos](https://github.com/anshug/claude-mythos) | Prompt framework that makes LLMs discover real, exploitable vulnerabilities. Six specialized agents covering recon, h... | 60 | +0 / +0 | `Research / Benchmark` | `Unknown` | `CC-BY-4.0` | 45 |
| 🆕 🧠 [yeasy/ai_security_guide](https://github.com/yeasy/ai_security_guide) | 从原理到实践，全面掌握大语言模型安全攻防之道 | 109 | +0 / +0 | `Guide / Standard` | `Python` | `Unknown` | 38 |
| 🆕 [agenticsorg/agentic-security](https://github.com/agenticsorg/agentic-security) | No description. | 59 | +0 / +0 | `Tool / Framework` | `TypeScript` | `Unknown` | 20 |

### Sandboxing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🏆 💎 [anthropics/defending-code-reference-harness](https://github.com/anthropics/defending-code-reference-harness) | Skills for threat modeling, scanning, triage, patching, plus an autonomous scanning harness you can /customize | 7,526 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 52 |
| 🆕 🧠 [digitranslab/allama](https://github.com/digitranslab/allama) | 🔥🔥🔥 AI security automation platform. Build visual workflows, deploy autonomous agents, and automate threat detection ... | 200 | +0 / +0 | `Tool / Framework` | `Python` | `AGPL-3.0` | 44 |
| 🆕 🧠 [controlplaneio/sandbox-probe](https://github.com/controlplaneio/sandbox-probe) | Agentic sandbox enumeration: find security issues, and seed escape automation. | 27 | +0 / +0 | `Tool / Framework` | `Go` | `Apache-2.0` | 43 |
| 🆕 🧠 [scadastrangelove/rust-in-peace](https://github.com/scadastrangelove/rust-in-peace) | rust-in-peace — agentic security review for Rust: autonomous find/triage/patch for unsafe/FFI memory bugs, panic-DoS,... | 21 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 37 |
| 🆕 🧠 [sundarsub/execwall](https://github.com/sundarsub/execwall) | OpenClaw Execution Firewall - Seccomp-locked AI agent sandbox with policy-enforced command governance, WhatsApp/Teleg... | 10 | +0 / +0 | `Research / Benchmark` | `Rust` | `Apache-2.0` | 36 |
| 🆕 🧠 [FareedKhan-dev/agentic-security-pipeline](https://github.com/FareedKhan-dev/agentic-security-pipeline) | An autonomous agentic pipeline that finds, proves, and patches real C memory-safety vulnerabilities end-to-end using ... | 18 | +0 / +0 | `Tool / Framework` | `Jupyter Notebook` | `MIT` | 36 |
| 🆕 [Harrishayy/Cereberus](https://github.com/Harrishayy/Cereberus) | Built with Cisco: The future of agent security (enforced at the kernel level) | 14 | +0 / +0 | `Research / Benchmark` | `Go` | `NOASSERTION` | 27 |
| 🆕 [jmagly/agentic-sandbox](https://github.com/jmagly/agentic-sandbox) | Self-hostable runtime for persistent autonomous coding agents — KVM-isolated VMs (or rootless containers), A2A-protoc... | 12 | +0 / +0 | `Tool / Framework` | `Rust` | `AGPL-3.0` | 25 |
| 🆕 [katosh/agent_sandbox](https://github.com/katosh/agent_sandbox) | Sandbox AI agents on HPC systems with slurm. | 12 | +0 / +0 | `Research / Benchmark` | `Shell` | `MIT` | 23 |
| 🆕 [seznam/jailoc](https://github.com/seznam/jailoc) | 🔒 Jail your AI agents — sandboxed Docker environments with network isolation for Opencode agents | 33 | +0 / +0 | `Tool / Framework` | `Go` | `MIT` | 20 |
| 🆕 [scitix/Agent-Sandbox](https://github.com/scitix/Agent-Sandbox) | Fast, Multi-Cloud Sandbox Engine for AI Agents | 11 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `Apache-2.0` | 20 |
| 🆕 [1996fanrui/agents-sandbox](https://github.com/1996fanrui/agents-sandbox) | Run AI agents(Claude Code, Codex, OpenClaw, etc.) with Full Power in Isolated Local Sandbox. Zero cost, Zero risk to ... | 24 | +0 / +0 | `Tool / Framework` | `Go` | `Apache-2.0` | 19 |
| 🆕 [CodeDuet/codeduet-microvm-ai-agent-sandbox](https://github.com/CodeDuet/codeduet-microvm-ai-agent-sandbox) | MicroVM AI agent sandbox system using Cloud Hypervisor and Python with support for both Linux and Windows guest opera... | 10 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 18 |

### Tool / Function Calling

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [samugit83/redamon](https://github.com/samugit83/redamon) | An AI-powered agentic red team framework that automates offensive security operations, from reconnaissance to exploit... | 2,598 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 71 |
| 🆕 🧠 [xyun92/trilane](https://github.com/xyun92/trilane) | TriLane: autonomous AI pentester and multi-agent security auditor for authorized gray-box testing of web apps and API... | 28 | +0 / +0 | `Tool / Framework` | `Rust` | `Apache-2.0` | 38 |
| 🆕 [PensiveFei/dsh-secure-audit](https://github.com/PensiveFei/dsh-secure-audit) | Read-only security & compliance plugin for DeepSeek Harness: prompt-injection detection, Chinese-PII redaction, and l... | 86 | +0 / +0 | `Awesome List` | `JavaScript` | `MIT` | 22 |

[Back to top](#ai-security-repository-radar)

---

<a id="llm-application-security"></a>

## 🛡️ LLM Application Security

**Repositories:** 99  
**Labs:** 8  
**Stars:** 23,618  
**7-day growth:** +0

### Benchmarks / Eval

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧠 [Virtue-Research/guard-eval-harness](https://github.com/Virtue-Research/guard-eval-harness) | One command to benchmark AI guardrails and coding agents across safety, security, jailbreak, prompt-injection, and se... | 19 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 49 |
| 🆕 ✅ [OWASP/www-project-llm-verification-standard](https://github.com/OWASP/www-project-llm-verification-standard) | OWASP LLM Security Verification Standard | 58 | +0 / +0 | `Research / Benchmark` | `Unknown` | `CC-BY-SA-4.0` | 39 |
| 🆕 [liu00222/Open-Prompt-Injection](https://github.com/liu00222/Open-Prompt-Injection) | This repository provides a benchmark for prompt injection attacks and defenses in LLMs | 503 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 37 |
| 🆕 ✅ [facebookresearch/rl-injector](https://github.com/facebookresearch/rl-injector) | Official release of code for the paper RL is a hammer and LLMs are nails A simple RL approach to stronger prompt inje... | 57 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 37 |
| 🆕 [DaoyuanLi2816/RepoGuardBench](https://github.com/DaoyuanLi2816/RepoGuardBench) | Benchmarking repository-borne prompt injection attacks and lightweight defenses for local coding agents. DL4C @ ICML ... | 70 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 34 |
| 🆕 ✅ [facebookresearch/SecAlign](https://github.com/facebookresearch/SecAlign) | Repo for the research paper "SecAlign: Defending Against Prompt Injection with Preference Optimization" | 102 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 33 |
| 🆕 ✅ [facebookresearch/prompt-siren](https://github.com/facebookresearch/prompt-siren) | A research workbench for developing and testing attacks against large language models, with a focus on prompt injecti... | 62 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 33 |
| 🆕 [role-confusion/prompt-injection-as-role-confusion](https://github.com/role-confusion/prompt-injection-as-role-confusion) | Prompt Injection as Role Confusion | 131 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 28 |
| 🆕 [SaFo-Lab/JailBreakV_28K](https://github.com/SaFo-Lab/JailBreakV_28K) | [COLM 2024] JailBreakV-28K: A comprehensive benchmark designed to evaluate the transferability of LLM jailbreak attac... | 99 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 27 |
| 🆕 [Dormant-Neurons/llm-pitfalls](https://github.com/Dormant-Neurons/llm-pitfalls) | Artifacts for the NDSS'26 paper "Chasing Shadows: Pitfalls in LLM Security Research" | 14 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 26 |
| 🆕 [usail-hkust/JailTrickBench](https://github.com/usail-hkust/JailTrickBench) | Bag of Tricks: Benchmarking of Jailbreak Attacks on LLMs. Empirical tricks for LLM Jailbreaking. (NeurIPS 2024) | 168 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 24 |
| 🆕 [SproutNan/GuidedBench](https://github.com/SproutNan/GuidedBench) | The official repository for guided jailbreak benchmark | 32 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 22 |
| 🆕 [listen0425/Safety-Layers](https://github.com/listen0425/Safety-Layers) | code space of paper "Safety Layers in Aligned Large Language Models: The Key to LLM Security" (ICLR 2025) | 26 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 22 |
| 🆕 [Sizhe-Chen/StruQ](https://github.com/Sizhe-Chen/StruQ) | official implementation of [USENIX Sec'25] StruQ: Defending Against Prompt Injection with Structured Queries | 78 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 21 |
| 🆕 [Norrrrrrr-lyn/WAInjectBench](https://github.com/Norrrrrrr-lyn/WAInjectBench) | Benchmarking prompt injection detections for web agents. | 23 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 21 |
| 🆕 [xndong/SATA](https://github.com/xndong/SATA) | Repository of SATA paradigm for LLM jailbreak. | 10 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 21 |
| 🆕 [theshi-1128/jailbreak-bench](https://github.com/theshi-1128/jailbreak-bench) | The most comprehensive and accurate LLM jailbreak attack benchmark by far | 22 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 20 |
| 🆕 [yihedeng9/DuoGuard](https://github.com/yihedeng9/DuoGuard) | DuoGuard: A Two-Player RL-Driven Framework for Multilingual LLM Guardrails | 34 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 19 |

### Dashboard / Platform

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [tomabai/llm-sec](https://github.com/tomabai/llm-sec) | A hands-on learning platform for understanding and testing LLM security vulnerabilities | 26 | +0 / +0 | `Guide / Standard` | `TypeScript` | `MIT` | 24 |

### Framework / Library

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧠 [mastepanoski/claude-skills](https://github.com/mastepanoski/claude-skills) | UX/UI evaluation, AI governance, and AI security skills for AI coding assistants. Audit interfaces with Nielsen heuri... | 54 | +0 / +0 | `Research / Benchmark` | `JavaScript` | `MIT` | 43 |
| 🆕 [Hellsender01/LLMMap](https://github.com/Hellsender01/LLMMap) | Automated prompt injection testing framework for LLM-integrated applications with dual-LLM architecture. | 207 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 28 |
| 🆕 [NextWordDev/psychoevals](https://github.com/NextWordDev/psychoevals) | Repository for PsychoEvals - a framework for LLM security, psychoanalysis, and moderation. | 18 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 22 |

### Guardrails

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [yatuk/tamga](https://github.com/yatuk/tamga) | Self-hosted LLM security proxy. PII redaction, prompt injection defense, KVKK/GDPR/PCI-DSS compliance. Sub-millisecon... | 37 | +0 / +0 | `Research / Benchmark` | `Go` | `AGPL-3.0` | 55 |
| 🆕 💎 🧠 [arekusandr/last_layer](https://github.com/arekusandr/last_layer) | Ultra-fast, low latency LLM prompt injection/jailbreak detection ⛓️ | 135 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 53 |
| 🆕 💎 🧠 [genia-dev/vibraniumdome](https://github.com/genia-dev/vibraniumdome) | LLM Security Platform. | 29 | +0 / +0 | `Research / Benchmark` | `Python` | `GPL-3.0` | 51 |
| 🆕 💎 🧠 [atliq/guardex-ai](https://github.com/atliq/guardex-ai) | Python SDK for LLM guardrails with safety classification, PII detection, prompt injection defense, and grounding chec... | 28 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 51 |
| 🆕 🧠 [Mattral/guardrail-rs](https://github.com/Mattral/guardrail-rs) | A zero-Python, production-grade LLM security layer written in Rust. | 13 | +0 / +0 | `Research / Benchmark` | `Rust` | `Apache-2.0` | 47 |
| 🆕 🧠 [MaxMLang/pytector](https://github.com/MaxMLang/pytector) | Easy to use LLM Prompt Injection Detection and Prompt Input Sanitization / Detector Python Package with support for l... | 45 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 43 |
| 🆕 🧠 [0x0pointer/seraph](https://github.com/0x0pointer/seraph) | seraph AI Guardrails "You do not truly know someone until you fight them" | 12 | +0 / +0 | `Research / Benchmark` | `Python` | `AGPL-3.0` | 36 |
| 🆕 🧠 [amk9978/Guardian](https://github.com/amk9978/Guardian) | Extensible Go microkernel for LLM guardrails, quota enforcement and policy plugins. | 10 | +0 / +0 | `Project` | `Go` | `Unknown` | 35 |
| 🆕 🧠 [balavenkatesh3322/guardrails-demo](https://github.com/balavenkatesh3322/guardrails-demo) | LLM Security Project with Llama Guard | 11 | +0 / +0 | `Tool / Framework` | `Python` | `Unknown` | 34 |
| 🆕 [remorses/opencode-injection-guard](https://github.com/remorses/opencode-injection-guard) | Prompt injection detection plugin for OpenCode. Works with any LLM provider. Scans tool call outputs with a judge mod... | 17 | +0 / +0 | `Tool / Framework` | `TypeScript` | `Unknown` | 29 |
| 🆕 [SecureCodeWarrior/ai-security-rules](https://github.com/SecureCodeWarrior/ai-security-rules) | This repository contains security rule files designed to be used with AI-assisted developer tools. | 64 | +0 / +0 | `Guide / Standard` | `Unknown` | `Unknown` | 28 |
| 🆕 [PIsberg/vibetags](https://github.com/PIsberg/vibetags) | VibeTags is a Java annotation processor that acts as AI guardrails for code generation tools like Cursor, Claude, Gem... | 25 | +0 / +0 | `Tool / Framework` | `Java` | `MIT` | 27 |
| 🆕 [mattolson/agent-sandbox](https://github.com/mattolson/agent-sandbox) | Secure local dev environment for collaboration with AI coding agents | 205 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 23 |
| 🆕 [jagreehal/pydantic-ai-guardrails](https://github.com/jagreehal/pydantic-ai-guardrails) | Guardrails for Pydantic AI | 15 | +0 / +0 | `Tool / Framework` | `MDX` | `MIT` | 19 |

### Jailbreak Testing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [praetorian-inc/augustus](https://github.com/praetorian-inc/augustus) | LLM security testing framework for detecting prompt injection, jailbreaks, and adversarial attacks — 190+ probes, 28 ... | 294 | +0 / +0 | `Research / Benchmark` | `Go` | `Apache-2.0` | 51 |
| 🆕 🧠 [Ratila1/JGuardrails](https://github.com/Ratila1/JGuardrails) | 🛡️ Programmable Guardrails for LLM Applications in Java. A framework-agnostic toolkit for input/output validation, PI... | 16 | +0 / +0 | `Tool / Framework` | `Java` | `NOASSERTION` | 49 |
| 🆕 [langgptai/LLM-Jailbreaks](https://github.com/langgptai/LLM-Jailbreaks) | LLM Jailbreaks, ChatGPT, Claude, Llama, DAN Prompts, Prompt Leaking | 784 | +0 / +0 | `Awesome List` | `Unknown` | `Apache-2.0` | 38 |
| 🆕 [agencyenterprise/PromptInject](https://github.com/agencyenterprise/PromptInject) | PromptInject is a framework that assembles prompts in a modular fashion to provide a quantitative analysis of the rob... | 526 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 37 |
| 🆕 🧠 [mirseo/string-formatter](https://github.com/mirseo/string-formatter) | A high-performance string formatter written in Rust. This project detects and blocks LLM prompt injection and jailbre... | 10 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 35 |
| 🆕 [NeuralTrust/echo-chamber](https://github.com/NeuralTrust/echo-chamber) | Code and examples for Echo Chamber LLM Jailbreak. | 10 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `MIT` | 30 |
| 🆕 [ParzivalHack/Aegis.rs](https://github.com/ParzivalHack/Aegis.rs) | The first locally-hosted, open-source LLM security proxy, written completely in Rust | 48 | +0 / +0 | `Tool / Framework` | `Rust` | `MIT` | 28 |
| 🆕 [ShenzheZhu/JailDAM](https://github.com/ShenzheZhu/JailDAM) | [COLM 2025] JailDAM: Jailbreak Detection with Adaptive Memory for Vision-Language Model | 26 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 26 |
| 🆕 [dleerdefi/llm-security-auditor](https://github.com/dleerdefi/llm-security-auditor) | Universal LLM security auditor with automated jailbreak testing, DSPy optimization, and OWASP 2025-aligned attack pat... | 22 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 23 |
| 🆕 [fastino-ai/GLiGuard](https://github.com/fastino-ai/GLiGuard) | Fastino's LLM guardrail | 61 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Apache-2.0` | 23 |
| 🆕 [gally16/LLM-Jailbreaking-Guide](https://github.com/gally16/LLM-Jailbreaking-Guide) | LLM Jailbreaking Guide主流大语言模型越狱指南 | 160 | +0 / +0 | `Guide / Standard` | `Unknown` | `Unknown` | 23 |
| 🆕 [0din-ai/sidekick](https://github.com/0din-ai/sidekick) | 0DIN Sidekick is a Firefox/Chromium Add-on/Extension for AI security researchers that streamlines LLM jailbreak testi... | 27 | +0 / +0 | `Guide / Standard` | `JavaScript` | `MIT` | 22 |
| 🆕 [zroe1/xlab-ai-security](https://github.com/zroe1/xlab-ai-security) | An online AI security course created by UChicago's XLab | 41 | +0 / +0 | `Guide / Standard` | `Jupyter Notebook` | `Unknown` | 22 |
| 🆕 [bcdannyboy/PromptMatryoshka](https://github.com/bcdannyboy/PromptMatryoshka) | Multi-Provider LLM Jailbreak Research Framework | 13 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 21 |
| 🆕 [YihanWang617/LLM-Jailbreaking-Defense-Backtranslation](https://github.com/YihanWang617/LLM-Jailbreaking-Defense-Backtranslation) | Code for paper "Defending aginast LLM Jailbreaking via Backtranslation" | 34 | +0 / +0 | `Research / Benchmark` | `Python` | `BSD-3-Clause` | 20 |
| 🆕 [Asstar-X/JailPrompter](https://github.com/Asstar-X/JailPrompter) | 专注于 大语言模型（LLM）提示词攻防（Prompt Attack & Defense）与提示词越狱（Prompt Jailbreak）研究的开源项目。 | 22 | +0 / +0 | `Project` | `Unknown` | `Unknown` | 20 |
| 🆕 [YihanWang617/llm-jailbreaking-defense](https://github.com/YihanWang617/llm-jailbreaking-defense) | A lightweight library for large laguage model (LLM) jailbreaking defense. | 61 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 19 |

### LLM Labs

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 🧪 [dhammon/ai-goat](https://github.com/dhammon/ai-goat) | Learn AI security through a series of vulnerable LLM CTF challenges. No sign ups, no cloud fees, run everything local... | 366 | +0 / +0 | `Lab` | `Python` | `GPL-2.0` | 57 |
| 🆕 💎 🧠 🧪 [securelayer7/PROMPTPurify](https://github.com/securelayer7/PROMPTPurify) | Prompt-injection guardrail for LLM applications. Compact model that outperforms larger open-source guards. No regex, ... | 80 | +0 / +0 | `Lab` | `TypeScript` | `MIT` | 56 |
| 🆕 💎 🧠 🧪 [Esther7171/TryHackMe-Walkthroughs](https://github.com/Esther7171/TryHackMe-Walkthroughs) | 137+ TryHackMe walkthroughs covering web exploitation, privilege escalation, CVE analysis, container escapes, and AI/... | 79 | +0 / +0 | `Lab` | `Python` | `BSD-3-Clause` | 54 |
| 🆕 💎 🧠 🧪 [anpa1200/AI-PT-Lab](https://github.com/anpa1200/AI-PT-Lab) | Intentionally vulnerable AI security training lab for LLM agents — demonstrates OWASP LLM Top 10 2025: RAG injection,... | 12 | +0 / +0 | `Lab` | `Python` | `Unknown` | 54 |
| 🆕 🧠 🧪 [TrustAI-laboratory/LLM-Security-CTF](https://github.com/TrustAI-laboratory/LLM-Security-CTF) | Learn LLM/AI Security through a series of vulnerable LLM CTF challenges. No sign ups, all fees, everything on the web... | 20 | +0 / +0 | `Lab` | `Unknown` | `MIT` | 47 |
| 🆕 🧪 [pasquini-dario/project_mantis](https://github.com/pasquini-dario/project_mantis) | Project Mantis: Hacking Back the AI-Hacker; Prompt Injection as a Defense Against LLM-driven Cyberattacks | 123 | +0 / +0 | `Lab` | `Python` | `Unknown` | 31 |
| 🆕 🧪 [xaoLiu1222/cc-bos](https://github.com/xaoLiu1222/cc-bos) | CC-BOS: Classical Chinese + Bio-inspired Optimization Search for LLM Jailbreak (arXiv:2602.22983) | 42 | +0 / +0 | `Lab` | `Python` | `Unknown` | 17 |
| 🆕 🧪 [aivillage/llm_verification](https://github.com/aivillage/llm_verification) | LLM prompt attacks for hacker CTFs via CTFd. | 15 | +0 / +0 | `Lab` | `Python` | `Unknown` | 15 |

### Model Scanning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [hack-umbrella/LLM-Security-Toolkit](https://github.com/hack-umbrella/LLM-Security-Toolkit) | 把AI安全从"艺术"变成"科学" - 全面的LLM安全工具包 | 15 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 20 |

### Prompt Injection Defense

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [toby-bridges/api-relay-audit](https://github.com/toby-bridges/api-relay-audit) | Local security audit for AI API relays and LLM proxies: detects prompt injection, model substitution, tool-call rewri... | 859 | +0 / +0 | `Research / Benchmark` | `Python` | `AGPL-3.0` | 53 |
| 🆕 💎 [greshake/llm-security](https://github.com/greshake/llm-security) | New ways of breaking app-integrated LLMs | 2,140 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `MIT` | 53 |
| 🆕 🏆 💎 [LouisShark/chatgpt_system_prompt](https://github.com/LouisShark/chatgpt_system_prompt) | A collection of GPT system prompts and various prompt injection/leaking knowledge. | 10,778 | +0 / +0 | `Guide / Standard` | `HTML` | `MIT` | 51 |
| 🆕 🧠 [mdombrov-33/go-promptguard](https://github.com/mdombrov-33/go-promptguard) | LLM prompt injection detection for Go applications | 12 | +0 / +0 | `Research / Benchmark` | `Go` | `MIT` | 43 |
| 🆕 ✅ [trailofbits/anamorpher](https://github.com/trailofbits/anamorpher) | image scaling attacks for multi-modal prompt injection | 1,079 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 42 |
| 🆕 🧠 [Seezo-io/llm-security-101](https://github.com/Seezo-io/llm-security-101) | Delving into the Realm of LLM Security: An Exploration of Offensive and Defensive Tools, Unveiling Their Present Capa... | 172 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 40 |
| 🆕 🧠 [dropbox/llm-security](https://github.com/dropbox/llm-security) | Dropbox LLM Security research code and results | 257 | +0 / +0 | `Guide / Standard` | `Python` | `Apache-2.0` | 39 |
| 🆕 🧠 [AdityaBhatt3010/When-LinkedIn-Gmail-Obey-Hidden-AI-Prompts-Lessons-in-Indirect-Prompt-Injection](https://github.com/AdityaBhatt3010/When-LinkedIn-Gmail-Obey-Hidden-AI-Prompts-Lessons-in-Indirect-Prompt-Injection) | A real-world look at how hidden instructions in profiles and emails trick AI into unexpected outputs, revealing the s... | 16 | +0 / +0 | `Project` | `Unknown` | `MIT` | 37 |
| 🆕 [davidwillisowen/IPIM](https://github.com/davidwillisowen/IPIM) | Indirect Prompt Injection Methodology (IPIM) - A structured process which security professionals can use to find Indi... | 24 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 34 |
| 🆕 ✅ [google-research/camel-prompt-injection](https://github.com/google-research/camel-prompt-injection) | Code for the paper "Defeating Prompt Injections by Design" | 394 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `Apache-2.0` | 34 |
| 🆕 🧠 [grepstrength/WideOpenAI](https://github.com/grepstrength/WideOpenAI) | Short list of indirect prompt injection attacks for OpenAI-based models. | 39 | +0 / +0 | `Research / Benchmark` | `Unknown` | `MIT` | 31 |
| 🆕 [LegioX-Cyber-Technologies/DeceptivGuard](https://github.com/LegioX-Cyber-Technologies/DeceptivGuard) | Deception LLM Guardrail that deceives perceived adversarial queries instead of refusing them. | 14 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 31 |
| 🆕 [eastmountyxz/LLM-security](https://github.com/eastmountyxz/LLM-security) | 该资源主要总结和积累大模型（LLM）及ChatGPT相关的安全学术论文或技术博客，希望对自己和大家有所帮助 | 10 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 29 |
| 🆕 [lambblue/rui](https://github.com/lambblue/rui) | Real User Instruction: Black-Box Instruction Authentication Middleware Against Indirect Prompt Injection | 96 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 27 |
| 🆕 [forcesunseen/llm-hackers-handbook](https://github.com/forcesunseen/llm-hackers-handbook) | A guide to LLM hacking: fundamentals, prompt injection, offense, and defense | 200 | +0 / +0 | `Guide / Standard` | `Unknown` | `Unknown` | 27 |
| 🆕 [Eric-Terminal/Cognitive-Hijacking-in-Long-Context-LLMs](https://github.com/Eric-Terminal/Cognitive-Hijacking-in-Long-Context-LLMs) | A novel prompt injection method via forged internal states, discovered by a high school student. | 64 | +0 / +0 | `Research / Benchmark` | `Unknown` | `NOASSERTION` | 26 |
| 🆕 [Greysahy/ipiguard](https://github.com/Greysahy/ipiguard) | [EMNLP 2025 Oral] IPIGuard: A Novel Tool Dependency Graph-Based Defense Against Indirect Prompt Injection in LLM Agents | 25 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 25 |
| 🆕 [LukeChen-go/indirect-pia-detection](https://github.com/LukeChen-go/indirect-pia-detection) | The official implementation of the work "Can Indirect Prompt Injection Attacks Be Detected and Removed?" | 11 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 25 |
| 🆕 [SheltonLiu-N/Universal-Prompt-Injection](https://github.com/SheltonLiu-N/Universal-Prompt-Injection) | The official implementation of our pre-print paper "Automatic and Universal Prompt Injection Attacks against Large La... | 74 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 22 |
| 🆕 [leizhao7/livepi](https://github.com/leizhao7/livepi) | LivePI: More Realistic Benchmarking of Agents Against Indirect Prompt Injection | 10 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 22 |
| 🆕 [LLMSecurity/HouYi](https://github.com/LLMSecurity/HouYi) | The automated prompt injection framework for LLM-integrated applications. | 276 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 22 |
| 🆕 [darkfiv/PromptInjectionScanner](https://github.com/darkfiv/PromptInjectionScanner) | No description. | 131 | +0 / +0 | `Tool / Framework` | `Unknown` | `Unknown` | 21 |
| 🆕 [AdityaBhatt3010/BurpSuite-Lab-Indirect-Prompt-Injection](https://github.com/AdityaBhatt3010/BurpSuite-Lab-Indirect-Prompt-Injection) | Turned a product review into a silent assassin — Carlos never saw it coming. | 12 | +0 / +0 | `Project` | `Unknown` | `MIT` | 19 |
| 🆕 [Ziyang-Zhang-6657/Attn-Defense](https://github.com/Ziyang-Zhang-6657/Attn-Defense) | Code and data for paper: Attn-Defense: Attention-Guided Detection, Location and Removal for Indirect Prompt Injection | 12 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 19 |
| 🆕 [LLMPID/LLMPID-AS](https://github.com/LLMPID/LLMPID-AS) | LLM Prompt Injection Detection API Service PoC. | 10 | +0 / +0 | `Project` | `Go` | `AGPL-3.0` | 18 |
| 🆕 [FloTeu/mr-injector](https://github.com/FloTeu/mr-injector) | Mr. Injector is a GitHub repository designed to provide practical experience with Large Language Model (LLM) security... | 20 | +0 / +0 | `Project` | `Python` | `MIT` | 15 |

### RAG Poisoning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧠 [mirzayasirabdullahbaig07/PromptGuard-AI](https://github.com/mirzayasirabdullahbaig07/PromptGuard-AI) | PromptGuard is a single-file AI security platform that scans user prompts in real-time and classifies them as SAFE, S... | 42 | +0 / +0 | `Tool / Framework` | `HTML` | `Unknown` | 47 |
| 🆕 🧠 [Phantom8420/Aegis](https://github.com/Phantom8420/Aegis) | Production-grade LLM security firewall with bidirectional enforcement, adversarial input scanning, and real-time oper... | 22 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 44 |
| 🆕 [jthack/PIPE](https://github.com/jthack/PIPE) | Prompt Injection Primer for Engineers | 613 | +0 / +0 | `Guide / Standard` | `Unknown` | `Unknown` | 36 |
| 🆕 [Meirtz/Awesome-LLM-Jailbreak](https://github.com/Meirtz/Awesome-LLM-Jailbreak) | No description. | 35 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 33 |
| 🆕 [bountyyfi/invisible-prompt-injection](https://github.com/bountyyfi/invisible-prompt-injection) | No description. | 113 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 30 |
| 🆕 [sinanw/llm-security-prompt-injection](https://github.com/sinanw/llm-security-prompt-injection) | This project investigates the security of large language models by performing binary classification of a set of input... | 66 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `MIT` | 29 |
| 🆕 [andersmyrmel/vard](https://github.com/andersmyrmel/vard) | Lightweight prompt injection detection for LLM applications | 36 | +0 / +0 | `Tool / Framework` | `TypeScript` | `MIT` | 25 |
| 🆕 [ZJU-LLM-Safety/DARWIN](https://github.com/ZJU-LLM-Safety/DARWIN) | DARWIN is a self-evolving LLM jailbreak framework that grows a reusable strategy pool through external extraction, sa... | 73 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 25 |

### Sandboxing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧠 [UseAI-pro/openclaw-skills-security](https://github.com/UseAI-pro/openclaw-skills-security) | Curated, security-first OpenClaw skills (Markdown-based). Security audit skills - detect prompt injection, supply cha... | 72 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 40 |
| 🆕 [multikernel/sandlock](https://github.com/multikernel/sandlock) | The lightest AI sandbox. A process-based sandbox for Linux, no container, no VM, no privilege, no prompt injection | 423 | +0 / +0 | `Research / Benchmark` | `Rust` | `Apache-2.0` | 26 |

### Tool / Function Calling

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [aryakvnust/LLMSecGuard](https://github.com/aryakvnust/LLMSecGuard) | LLM Security Guard for Code | 22 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 15 |

[Back to top](#ai-security-repository-radar)

---

<a id="labs"></a>

## 🧪 Labs

**Repositories:** 2  
**Labs:** 2  
**Stars:** 82  
**7-day growth:** +0

### LLM Labs

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧠 🧪 ✅ [OWASP/www-project-promptme](https://github.com/OWASP/www-project-promptme) | PromptMe is an educational project that showcases security vulnerabilities in large language models (LLMs) and their ... | 39 | +0 / +0 | `Lab` | `Python` | `Apache-2.0` | 47 |
| 🆕 🧠 🧪 [aira-security/Vulnerable-AI-Chatbot](https://github.com/aira-security/Vulnerable-AI-Chatbot) | An intentionally vulnerable AI chatbot to learn and practice AI Security. | 43 | +0 / +0 | `Lab` | `HTML` | `MIT` | 35 |

[Back to top](#ai-security-repository-radar)

---

<a id="mcp-security"></a>

## 🧩 MCP Security

**Repositories:** 181  
**Labs:** 21  
**Stars:** 109,191  
**7-day growth:** +0

### Benchmarks / Eval

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [TalEliyahu/Awesome-AI-Security](https://github.com/TalEliyahu/Awesome-AI-Security) | Curated resources, research, and tools for securing AI systems | 873 | +0 / +0 | `Awesome List` | `Unknown` | `MIT` | 81 |
| 🆕 💎 🧠 [beyefendi/awesome-llm-security](https://github.com/beyefendi/awesome-llm-security) | Awesome LLM security tools, research, and documents | 39 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 80 |
| 🆕 💎 🧠 [beenuar/AiSOC](https://github.com/beenuar/AiSOC) | Open-source AI Security Operations Center: alert fusion, LLM-agent triage, MITRE ATT&CK investigation, and a replayab... | 2,369 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 78 |
| 🆕 💎 🧠 [HeadyZhang/agent-audit](https://github.com/HeadyZhang/agent-audit) | Static security scanner for LLM agents — prompt injection, MCP config auditing, taint analysis. 51 rules mapped to OW... | 233 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 67 |
| 🆕 💎 🧠 [HayatoFujihara/awesome-ai-red-teaming-jp](https://github.com/HayatoFujihara/awesome-ai-red-teaming-jp) | AI Red Teaming / AI Safety に関する日本語リソースのキュレーションリスト | 13 | +0 / +0 | `Awesome List` | `Python` | `CC0-1.0` | 63 |

### Dashboard / Platform

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [secureagentics/Adrian](https://github.com/secureagentics/Adrian) | Open-source runtime AI agent security tool - monitors and controls AI agents, catching malicious tool use, prompt inj... | 576 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 67 |
| 🆕 💎 🧠 [mythos-agent/mythos-agent](https://github.com/mythos-agent/mythos-agent) | The AI security agent guards your code. | 44 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 60 |
| 🆕 [mikeprivette/ai-security-shared-responsibility](https://github.com/mikeprivette/ai-security-shared-responsibility) | AI Security Shared Responsibility Model | 93 | +0 / +0 | `Tool / Framework` | `Unknown` | `MIT` | 31 |
| 🆕 [MikkoParkkola/nab](https://github.com/MikkoParkkola/nab) | Token-lean web microfetch for LLM agents: any URL → clean markdown via CLI, MCP server, and Claude Code plugin. Real ... | 13 | +0 / +0 | `Tool / Framework` | `Rust` | `MIT` | 27 |

### Framework / Library

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [akanthed/SecureAI-Scan](https://github.com/akanthed/SecureAI-Scan) | Find AI/LLM security vulnerabilities in your code before attackers do — covers prompt injection, MCP tool poisoning, ... | 22 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 62 |
| 🆕 🧠 [0-Vault/Vault-0](https://github.com/0-Vault/Vault-0) | Vault-0: Agent Security, Monitor & x402 Wallet for OpenClaw. Encrypted secret vault, real-time agent monitor, policy ... | 15 | +0 / +0 | `Tool / Framework` | `Svelte` | `MIT` | 34 |
| 🆕 [BakeLens/crust](https://github.com/BakeLens/crust) | 🌟 Open Source AI Agent Security Infrastructure — intercepts and blocks dangerous agent behaviors before they happen. ... | 440 | +0 / +0 | `Research / Benchmark` | `Go` | `NOASSERTION` | 33 |
| 🆕 [ruvnet/Agent-Name-Service](https://github.com/ruvnet/Agent-Name-Service) | Agent Name Service (ANS) Protocol, introduced by the OWASP GenAI Security Project, is a foundational framework design... | 82 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 22 |
| 🆕 [zhinjs/zhin](https://github.com/zhinjs/zhin) | AI-native TypeScript bot framework — one codebase for 20+ chat platforms (QQ, Discord, Telegram, Slack, WeChat…). Opt... | 136 | +0 / +0 | `Tool / Framework` | `TypeScript` | `MIT` | 22 |

### Guardrails

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [requie/LLMSecurityGuide](https://github.com/requie/LLMSecurityGuide) | A comprehensive reference for securing Large Language Models (LLMs). Covers OWASP GenAI Top-10 risks, prompt injectio... | 152 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 98 |
| 🆕 💎 🧠 [wearetyomsmnv/Awesome-LLMSecOps](https://github.com/wearetyomsmnv/Awesome-LLMSecOps) | LLM \| Agentic \| Security \| Operations in one github repo with good links and pictures. | 161 | +0 / +0 | `Awesome List` | `HTML` | `Unknown` | 96 |
| 🆕 🏆 💎 🧠 [superagent-ai/superagent](https://github.com/superagent-ai/superagent) | Superagent protects your AI applications against prompt injections, data leaks, and harmful outputs. Embed safety dir... | 6,754 | +0 / +0 | `Tool / Framework` | `TypeScript` | `MIT` | 73 |
| 🆕 💎 🧠 [webpro255/awesome-ai-agent-attacks](https://github.com/webpro255/awesome-ai-agent-attacks) | A curated timeline of real AI agent security incidents, breaches, and vulnerabilities (2024-2026). Every entry source... | 74 | +0 / +0 | `Awesome List` | `Unknown` | `MIT` | 71 |
| 🆕 💎 🧠 [myProjectsRavi/sentinel-protocol](https://github.com/myProjectsRavi/sentinel-protocol) | Open-source AI security firewall. 81 engines for PII detection, prompt injection defense, MCP security, and egress cl... | 17 | +0 / +0 | `Research / Benchmark` | `JavaScript` | `MIT` | 67 |
| 🆕 💎 🧠 [NuGuardAI/nuguard](https://github.com/NuGuardAI/nuguard) | AI red-teaming tool and LLM security framework to evaluate agentic AI applications. Tests prompt injections, handles ... | 50 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 61 |
| 🆕 💎 🧠 [votal-ai-hq/ai-red-teaming](https://github.com/votal-ai-hq/ai-red-teaming) | Whitebox & Blackbox AI red-teaming framework for LLMs & Agentic AI apps. It analyzes your app's source code to discov... | 33 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 60 |
| 🆕 💎 🧠 [PrismorSec/prismor](https://github.com/PrismorSec/prismor) | Self-hosted runtime control plane for AI agents. Observe or HITL approve or Block rogue tool calls before it executes... | 377 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 60 |
| 🆕 💎 🧠 [guardana/guardana](https://github.com/guardana/guardana) | Open-source AI security verification for model artifacts, live endpoints, MCP servers, and recorded agent traces. Rep... | 83 | +0 / +0 | `Research / Benchmark` | `HTML` | `Apache-2.0` | 59 |
| 🆕 💎 🧠 [kaplanlior/oss-llm-security](https://github.com/kaplanlior/oss-llm-security) | Curated list of Open Source project focused on LLM security | 85 | +0 / +0 | `Awesome List` | `Unknown` | `CC-BY-SA-4.0` | 56 |
| 🆕 🏆 💎 [ai-boost/awesome-prompts](https://github.com/ai-boost/awesome-prompts) | Curated list of chatgpt prompts from the top-rated GPTs in the GPTs Store. Prompt Engineering, prompt attack & prompt... | 8,941 | +0 / +0 | `Awesome List` | `Unknown` | `GPL-3.0` | 56 |
| 🆕 💎 🧠 [Acacian/aegis](https://github.com/Acacian/aegis) | LLM guardrails & prompt injection detection for Python. Auto-instruments LangChain, CrewAI, OpenAI, LiteLLM + 8 more ... | 16 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 55 |
| 🆕 💎 🧠 [emmanuelgjr/GenAI-Security-Crosswalk](https://github.com/emmanuelgjr/GenAI-Security-Crosswalk) | The most comprehensive open-source mapping of OWASP GenAI risks to industry frameworks - 70 mapping files, 25 framewo... | 13 | +0 / +0 | `Research / Benchmark` | `JavaScript` | `NOASSERTION` | 52 |
| 🆕 🧠 [killertcell428/ai-guardian](https://github.com/killertcell428/ai-guardian) | 🛡️ Open-source LLM security library for Python. Protect your AI apps from prompt injection, data leakage, and harmful... | 14 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 45 |
| 🆕 ✅ [trailofbits/mcp-context-protector](https://github.com/trailofbits/mcp-context-protector) | MCP security wrapper | 226 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 44 |
| 🆕 🧠 [gy15901580825/Argus](https://github.com/gy15901580825/Argus) | Black-box, open-source red-team testing for AI agents. Point it at any HTTP, gRPC, or browser-using agent endpoint; r... | 205 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 39 |

### Jailbreak Testing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🏆 💎 🧠 ✅ [Tencent/AI-Infra-Guard](https://github.com/Tencent/AI-Infra-Guard) | A full-stack AI Red Teaming platform securing AI ecosystems via Agent Scan, Skills Scan, MCP scan, AI Infra scan and ... | 6,593 | +0 / +0 | `Awesome List` | `Python` | `Apache-2.0` | 112 |
| 🆕 💎 🧠 [requie/AI-Red-Teaming-Guide](https://github.com/requie/AI-Red-Teaming-Guide) | A comprehensive guide to adversarial testing and security evaluation of AI systems, helping organizations identify vu... | 894 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 76 |
| 🆕 💎 🧠 [preambleai/prompt-injector](https://github.com/preambleai/prompt-injector) | AI security and prompt injection payload toolkit | 33 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `Apache-2.0` | 70 |
| 🆕 💎 🧠 [getagentseal/agentseal](https://github.com/getagentseal/agentseal) | Security toolkit for AI agents. Scan your machine for dangerous skills and MCP configs, monitor for supply chain atta... | 376 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 70 |
| 🆕 💎 🧠 [fevziegeyurtsevenler/LLM-Security-Turkiye](https://github.com/fevziegeyurtsevenler/LLM-Security-Turkiye) | Türkçe yapay zeka güvenliği için açık kaynak çatı: rehber serisi, uygulamalı akademi, araştırma & deneyler, 23+ araç ... | 11 | +0 / +0 | `Research / Benchmark` | `Unknown` | `CC-BY-4.0` | 65 |
| 🆕 💎 🧠 [hardw00t/ai-security-arsenal](https://github.com/hardw00t/ai-security-arsenal) | A collection of skills, agents, commands, and workflows for security researchers. Compatible with Claude Code, Claude... | 104 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 57 |
| 🆕 💎 🧠 [ZeroLeaks/zeroleaks](https://github.com/ZeroLeaks/zeroleaks) | AI Security Scanner - Test your AI systems for prompt injection and extraction vulnerabilities | 736 | +0 / +0 | `Tool / Framework` | `TypeScript` | `NOASSERTION` | 57 |
| 🆕 💎 🧠 [dodoguardai/dodoguard](https://github.com/dodoguardai/dodoguard) | DodoGuard: Full‑lifecycle agent security for Dify, Coze, FastGPT, n8n, Tencent ADP, ByteDance HiAgent, and more. | 142 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 51 |
| 🆕 🧠 [seojoonkim/prompt-guard](https://github.com/seojoonkim/prompt-guard) | Advanced prompt injection defense system for AI agents. Multi-language detection, severity scoring, and security audi... | 178 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 43 |
| 🆕 🧠 [agentshield-ai/sigma-ai](https://github.com/agentshield-ai/sigma-ai) | Sigma detection rules for AI agent security monitoring | 15 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 39 |
| 🆕 🧠 [Josh-blythe/bordair-multimodal](https://github.com/Josh-blythe/bordair-multimodal) | Open-source cross-modal and multimodal prompt injection test suite. 250,000+ attack payloads across text, image, docu... | 76 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 36 |
| 🆕 [bob-reis/claude-pentest-skills](https://github.com/bob-reis/claude-pentest-skills) | Coleção de skills de segurança ofensiva para Claude Code metodologia PTES completa com AWS/IAM (WorstAssume), pfSense... | 10 | +0 / +0 | `Tool / Framework` | `Python` | `Unknown` | 34 |
| 🆕 🧠 [numbergroup/AgentGuard](https://github.com/numbergroup/AgentGuard) | A+ Grade AI Agent Security Framework - Military-grade protection against prompt injection, command injection, and Uni... | 102 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 33 |
| 🆕 [awiseguy88/openclaw-advanced-prompt-injection-defense-system](https://github.com/awiseguy88/openclaw-advanced-prompt-injection-defense-system) | this is the first openclaw prompy injection defense system | 23 | +0 / +0 | `Project` | `Unknown` | `Unknown` | 24 |

### MCP Labs

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 🧪 [ottosulin/awesome-ai-security](https://github.com/ottosulin/awesome-ai-security) | A collection of awesome resources related AI security | 1,498 | +0 / +0 | `Lab` | `Unknown` | `MIT` | 101 |
| 🆕 💎 🧠 🧪 [sonuoffsec/AI-Security-Hub](https://github.com/sonuoffsec/AI-Security-Hub) | A comprehensive AI Security Hub featuring payloads, cheat sheets, hands-on labs, security tools, MCP security, RAG se... | 18 | +0 / +0 | `Lab` | `Unknown` | `MIT` | 98 |
| 🆕 💎 🧠 🧪 [scadastrangelove/awesome-ai-security-tools](https://github.com/scadastrangelove/awesome-ai-security-tools) | A curated list of public-source, research, and commercial tools for AI security and AI-assisted cybersecurity — autot... | 1,546 | +0 / +0 | `Lab` | `Python` | `NOASSERTION` | 97 |
| 🆕 💎 🧠 🧪 [brinhosa/awesome-ai-security](https://github.com/brinhosa/awesome-ai-security) | A collection of awesome AI Security, LLM Security, and Prompt Injection tools and resources. | 37 | +0 / +0 | `Lab` | `Unknown` | `CC0-1.0` | 90 |
| 🆕 💎 🧠 🧪 [ProjectRecon/awesome-ai-agents-security](https://github.com/ProjectRecon/awesome-ai-agents-security) | A living map of the AI agent security ecosystem. | 71 | +0 / +0 | `Lab` | `Unknown` | `NOASSERTION` | 86 |
| 🆕 💎 🧠 🧪 [anmolksachan/AI-ML-Free-Resources-for-Security-and-Prompt-Injection](https://github.com/anmolksachan/AI-ML-Free-Resources-for-Security-and-Prompt-Injection) | AI/ML Pentesting Roadmap for Beginners | 794 | +0 / +0 | `Lab` | `Unknown` | `Unknown` | 84 |
| 🆕 💎 🧠 🧪 [DmitrL-dev/AISecurity](https://github.com/DmitrL-dev/AISecurity) | Open-source AI security lab: EN/RU academy, public Rust defenses, research and local input evaluation. Explore Specto... | 111 | +0 / +0 | `Lab` | `Python` | `NOASSERTION` | 82 |
| 🆕 💎 🧠 🧪 [sonuoffsec/DVAP](https://github.com/sonuoffsec/DVAP) | An OWASP-aligned intentionally vulnerable platform for learning and testing AI, LLM, RAG, MCP, and Agentic AI security. | 29 | +0 / +0 | `Lab` | `TypeScript` | `NOASSERTION` | 75 |
| 🆕 💎 🧠 🧪 [gmh5225/awesome-ai-security](https://github.com/gmh5225/awesome-ai-security) | A curated list of AI Security materials and resources for Pentesters, Bug Hunters, and Security Researchers. | 50 | +0 / +0 | `Lab` | `Unknown` | `MIT` | 74 |
| 🆕 💎 🧠 🧪 [fevziegeyurtsevenler/awesome-ai-security-tr](https://github.com/fevziegeyurtsevenler/awesome-ai-security-tr) | Türkçe yapay zeka güvenliği kaynakları — prompt injection, jailbreak, red teaming, guardrail, ajan/MCP ve RAG güvenli... | 13 | +0 / +0 | `Lab` | `Python` | `CC-BY-4.0` | 73 |
| 🆕 💎 🧠 🧪 [Joe-B-Security/awesome-prompt-injection](https://github.com/Joe-B-Security/awesome-prompt-injection) | Learn about a type of vulnerability that specifically targets machine learning models | 656 | +0 / +0 | `Lab` | `Unknown` | `CC0-1.0` | 72 |
| 🆕 💎 🧠 🧪 [Yeti-791/Awesome-Offensive-AI-Agentic-Landscape](https://github.com/Yeti-791/Awesome-Offensive-AI-Agentic-Landscape) | This document curates open-source projects, academic papers, capability benchmarks, and commercial solutions (interna... | 310 | +0 / +0 | `Lab` | `Unknown` | `MIT` | 65 |
| 🆕 💎 🧠 🧪 [Arcanum-Sec/ai-sec-resources](https://github.com/Arcanum-Sec/ai-sec-resources) | AI Security Resources Hub | 506 | +0 / +0 | `Lab` | `HTML` | `Unknown` | 62 |
| 🆕 💎 🧠 🧪 [Threekiii/Awesome-AI-Redteam](https://github.com/Threekiii/Awesome-AI-Redteam) | 一个 AI 攻防知识库。A knowledge base for AI security. | 51 | +0 / +0 | `Lab` | `Unknown` | `Unknown` | 60 |
| 🆕 💎 🧠 🧪 [hi-unc1e/Auto_JB_APE](https://github.com/hi-unc1e/Auto_JB_APE) | 引擎就一条回路：侦察 → 规划 → 提交 → 判定 → 变异。纯标准库，零框架依赖。内置裁判打 S/A/B/C，canary 或正则命中才算赢，没坐实的不提交。334 个测试离线全绿。CLI、Python API、MCP。One lo... | 13 | +0 / +0 | `Lab` | `Python` | `MIT` | 52 |
| 🆕 🧠 🧪 [EvanThomasLuke/Awesome-AI-Security-Skills](https://github.com/EvanThomasLuke/Awesome-AI-Security-Skills) | A list of AI security skills | 55 | +0 / +0 | `Lab` | `Unknown` | `Unknown` | 49 |
| 🆕 🧠 🧪 [mcp-security-project/awesome-agentic-mcp-security](https://github.com/mcp-security-project/awesome-agentic-mcp-security) | Awesome Agentic MCP Security List | 21 | +0 / +0 | `Lab` | `Unknown` | `CC0-1.0` | 48 |
| 🆕 🧠 🧪 [ZeroDayEvil/ai-security-tool](https://github.com/ZeroDayEvil/ai-security-tool) | 🛡️ Free open-source AI-powered security terminal & vulnerability scanner (CVE, SBOM). Supports SSH, SFTP, RDP, VNC, S... | 414 | +0 / +0 | `Lab` | `HTML` | `MIT` | 45 |
| 🆕 🧠 🧪 [smartdevil09/AI-Security-Professional-Roadmap](https://github.com/smartdevil09/AI-Security-Professional-Roadmap) | No description. | 33 | +0 / +0 | `Lab` | `Unknown` | `Unknown` | 40 |
| 🆕 🧠 🧪 [dwarvesf/claude-guardrails](https://github.com/dwarvesf/claude-guardrails) | Hardened security configuration for Claude Code; permission deny rules, shell hooks, and prompt injection defense in ... | 36 | +0 / +0 | `Lab` | `Shell` | `MIT` | 38 |
| 🆕 🧪 [PawelKozy/mcp-breach-to-fix-labs](https://github.com/PawelKozy/mcp-breach-to-fix-labs) | Hands-on MCP security lab: 10 real incidents reproduced with vulnerable/secure MCP servers, pytest regressions, and C... | 89 | +0 / +0 | `Lab` | `Python` | `Unknown` | 22 |

### Model Scanning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [RiccardoBiosas/awesome-MLSecOps](https://github.com/RiccardoBiosas/awesome-MLSecOps) | A curated list of MLSecOps tools and resources for securing machine learning and AI systems - adversarial ML defense,... | 473 | +0 / +0 | `Awesome List` | `Astro` | `MIT` | 120 |

### Prompt Injection Defense

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [muellerberndt/awesome-ai-security](https://github.com/muellerberndt/awesome-ai-security) | An AI security awesome list / learning journey | 131 | +0 / +0 | `Awesome List` | `TypeScript` | `Unknown` | 84 |
| 🆕 💎 🧠 [lasso-security/claude-hooks](https://github.com/lasso-security/claude-hooks) | Lasso security integrations for Claude Code, including prompt-injection defenses | 267 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 50 |
| 🆕 🧠 [fubak/ferret-scan](https://github.com/fubak/ferret-scan) | Security scanner for LLM CLI (Claude Code, Codex, Gemini, Droid, Opencode, etc) configurations and MD files - detect ... | 82 | +0 / +0 | `Tool / Framework` | `TypeScript` | `MIT` | 45 |
| 🆕 🧠 [xamitgupta/ipi-scanner](https://github.com/xamitgupta/ipi-scanner) | Detect Indirect Prompt Injection attacks before your LLM reads them | 15 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 43 |
| 🆕 🧠 [EresusSecurity/Eresus-sentinel](https://github.com/EresusSecurity/Eresus-sentinel) | AI/LLM security scanner — model artifact analysis, prompt injection firewall, MCP agent validation, pickle/safetensor... | 13 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 41 |
| 🆕 [hswtnb-blip/prompt-authgate](https://github.com/hswtnb-blip/prompt-authgate) | Claude Code prompt injection defense via source authentication tokens. UserPromptSubmit hookでユーザー入力にのみ認証トークンを付与し、ファイル... | 15 | +0 / +0 | `Project` | `Shell` | `MIT` | 15 |

### RAG Poisoning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [jassics/awesome-genai-security](https://github.com/jassics/awesome-genai-security) | Curated list of links, references, books videos, tutorials (Free or Paid), Exploit, CTFs, Hacking Practices etc. whic... | 77 | +0 / +0 | `Awesome List` | `Unknown` | `GPL-3.0` | 93 |
| 🆕 💎 🧠 [4vanish/AI-Pentest-Playbook](https://github.com/4vanish/AI-Pentest-Playbook) | 🛡 The reference playbook for pentesting AI chatbots & LLM-powered apps in one place. Ready-to-use payloads covering t... | 36 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 77 |
| 🆕 💎 🧠 [SnailSploit/AATMF-Adversarial-AI-Threat-Modeling-Framework](https://github.com/SnailSploit/AATMF-Adversarial-AI-Threat-Modeling-Framework) | AATMF \| An Open Source - Adversarial AI Threat Modeling Framework | 41 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 69 |
| 🆕 💎 🧠 [GoldenWing-360/claude-security-skills](https://github.com/GoldenWing-360/claude-security-skills) | 25 production-tested defensive security skills for Claude Code - WordPress, VPS, Cloudflare, Next.js hardening, AI ag... | 19 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 60 |
| 🆕 💎 🧠 [GenAI-Security-Project/GenAI-Red-Team-Lab](https://github.com/GenAI-Security-Project/GenAI-Red-Team-Lab) | Provides a collection of sandboxes, exploitation code, and tutorials that exemplifies GenAI Red Teaming exercises. It... | 54 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 59 |
| 🆕 💎 🧠 [calmrocks/ai-engineer-notebooks](https://github.com/calmrocks/ai-engineer-notebooks) | Hands-on, framework-free Colab notebooks for the AI Engineer / Forward Deployed Engineer (FDE) skill set — model APIs... | 642 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `MIT` | 51 |
| 🆕 🧠 [9hannahnine-jpg/arc-gate](https://github.com/9hannahnine-jpg/arc-gate) | Arc Gate — LLM proxy with prompt injection detection. Bendex Geometry. | 11 | +0 / +0 | `Research / Benchmark` | `Python` | `NOASSERTION` | 49 |
| 🆕 🧠 [nshportun/BestTester](https://github.com/nshportun/BestTester) | Production-grade Playwright + TypeScript QA framework with AI-powered testing, LLM-as-Judge evaluation, MCP server, 7... | 18 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `Apache-2.0` | 37 |
| 🆕 [wrxck/srag](https://github.com/wrxck/srag) | Semantic code search and RAG system written in Rust with tree-sitter chunking, MCP server for IDE integration, prompt... | 21 | +0 / +0 | `Tool / Framework` | `Rust` | `GPL-3.0` | 34 |
| 🆕 🧠 [damienbod/McpSecurity](https://github.com/damienbod/McpSecurity) | Research MCP, OAuth, security | 21 | +0 / +0 | `Tool / Framework` | `C#` | `Apache-2.0` | 33 |
| 🆕 🧠 [nuryslyrt/AISecTips-Tricks](https://github.com/nuryslyrt/AISecTips-Tricks) | AI Red Team & Blue Team Tips & Tricks! | 10 | +0 / +0 | `Project` | `Unknown` | `GPL-3.0` | 30 |

### Sandboxing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [bridge-mind/BridgeWard](https://github.com/bridge-mind/BridgeWard) | Trust nothing. Ship safely. — Skeptical-reading and prompt-injection defense skill for AI agents. Provenance tagging,... | 42 | +0 / +0 | `Research / Benchmark` | `Shell` | `MIT` | 61 |
| 🆕 🧠 [kajogo777/the-agent-sandbox-taxonomy](https://github.com/kajogo777/the-agent-sandbox-taxonomy) | An open taxonomy and scoring framework for evaluating AI agent sandboxes: 7 defense layers, 7 threat categories, 3 ev... | 103 | +0 / +0 | `Research / Benchmark` | `Go` | `Unknown` | 45 |
| 🆕 ✅ [Azure/kars](https://github.com/Azure/kars) | Agent Reference Stack for Kubernetes (kars) - an open source stack from Microsoft for running AI agents safely on Kub... | 44 | +0 / +0 | `Tool / Framework` | `Rust` | `MIT` | 44 |
| 🆕 🧠 [Gach0ng/AgentStalker](https://github.com/Gach0ng/AgentStalker) | A standardized agent vulnerability benchmark, formal taint tracking proofs, and a cross-language unified AST model, i... | 130 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 38 |
| 🆕 [schmitthub/clawker](https://github.com/schmitthub/clawker) | AI coding agent sandbox platform | 55 | +0 / +0 | `Tool / Framework` | `Go` | `MIT` | 34 |
| 🆕 🧠 [h9-tec/agentic-rag-security-lab](https://github.com/h9-tec/agentic-rag-security-lab) | No description. | 16 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 33 |
| 🆕 [kstenerud/yoloai](https://github.com/kstenerud/yoloai) | Your agent is a security risk, so treat it like one. yoloAI does AI agent sandboxing right. | 213 | +0 / +0 | `Tool / Framework` | `Go` | `MIT` | 31 |
| 🆕 [fishman/awesome-agent-sandbox](https://github.com/fishman/awesome-agent-sandbox) | Portable, self-contained sandboxes for AI coding agents - microVMs, containers, isolation harnesses. | 18 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 29 |
| 🆕 [arjan/awesome-agent-sandboxes](https://github.com/arjan/awesome-agent-sandboxes) | A curated list of code-execution sandboxing solutions for AI/LLM agents | 46 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 27 |

### Tool / Function Calling

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [frangelbarrera/Awesome-Hacking-with-AI](https://github.com/frangelbarrera/Awesome-Hacking-with-AI) | A curated, evidence-led guide to AI for security and security for AI. Defensive research, agent safety, MCP security,... | 27 | +0 / +0 | `Awesome List` | `Python` | `MIT` | 87 |
| 🆕 🏆 💎 🧠 ✅ [NVIDIA/SkillSpector](https://github.com/NVIDIA/SkillSpector) | Security scanner for AI agent skills. Detect vulnerabilities, malicious patterns, security risks, prompt injection, d... | 18,336 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 87 |
| 🆕 💎 🧠 [graygnatconsole/mcp-audit-tool](https://github.com/graygnatconsole/mcp-audit-tool) | 🛡️ Security audit CLI for Model Context Protocol (MCP) servers — scan AI agent configs for tool poisoning, rug pulls,... | 56 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 76 |
| 🆕 💎 🧠 [TalEliyahu/AI-Security-Newsletter](https://github.com/TalEliyahu/AI-Security-Newsletter) | AI Security Newsletter - A monthly digest of AI security research, insights, reports, upcoming events, and tools & re... | 48 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 67 |
| 🆕 💎 🧠 [sinewaveai/agent-security-scanner-mcp](https://github.com/sinewaveai/agent-security-scanner-mcp) | Security scanner MCP server for AI coding agents. Prompt injection firewall, package hallucination detection (4.3M+ p... | 122 | +0 / +0 | `Tool / Framework` | `JavaScript` | `MIT` | 65 |
| 🆕 💎 🧠 [Agent-Threat-Rule/agent-threat-rules](https://github.com/Agent-Threat-Rule/agent-threat-rules) | Open detection-rule standard for AI agent security threats — like Sigma, but for AI agents. Executable, testable rule... | 400 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 65 |
| 🆕 💎 🧠 [Puliczek/awesome-mcp-security](https://github.com/Puliczek/awesome-mcp-security) | 🔥🔒 Awesome MCP (Model Context Protocol) Security 🖥️ | 737 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 65 |
| 🆕 💎 🧠 [Coff0xc/LLM-Security-Assessment-Framework](https://github.com/Coff0xc/LLM-Security-Assessment-Framework) | FORGEDAN — An Evolutionary Framework for LLM Security Assessment \| 6 Attack Methods \| 18 Model Adapters \| Vue 3 Da... | 24 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 64 |
| 🆕 💎 🧠 [affaan-m/agentshield](https://github.com/affaan-m/agentshield) | AI agent security scanner. Detect vulnerabilities in agent configurations, MCP servers, and tool permissions. Availab... | 1,224 | +0 / +0 | `Tool / Framework` | `TypeScript` | `MIT` | 60 |
| 🆕 🏆 💎 [Portkey-AI/gateway](https://github.com/Portkey-AI/gateway) | A blazing fast AI Gateway with integrated guardrails. Route to 1,600+ LLMs, 50+ AI Guardrails with 1 fast & friendly ... | 13,084 | +0 / +0 | `Tool / Framework` | `TypeScript` | `MIT` | 60 |
| 🆕 💎 🧠 [luckyPipewrench/pipelock](https://github.com/luckyPipewrench/pipelock) | Open-source AI agent firewall for MCP security and agent egress. Scans mediated HTTP, MCP, A2A, and WebSocket traffic... | 906 | +0 / +0 | `Tool / Framework` | `Go` | `Apache-2.0` | 59 |
| 🆕 💎 🧠 ✅ [OWASP/AISVS](https://github.com/OWASP/AISVS) | The AI Security Verification Standard (AISVS) focuses on providing developers, architects, and security professionals... | 456 | +0 / +0 | `Tool / Framework` | `Unknown` | `CC-BY-SA-4.0` | 57 |
| 🆕 💎 🧠 [ASCIT31/Dark-Moon](https://github.com/ASCIT31/Dark-Moon) | Open-source autonomous AI penetration testing. 50 specialist agents across web, API, cloud, Active Directory, Kuberne... | 970 | +0 / +0 | `Research / Benchmark` | `Python` | `GPL-3.0` | 56 |
| 🆕 💎 🧠 [nshkrdotcom/GUARDRAIL](https://github.com/nshkrdotcom/GUARDRAIL) | GUARDRAIL - MCP Security - Gateway for Unified Access, Resource Delegation, and Risk-Attenuating Information Limits | 18 | +0 / +0 | `Tool / Framework` | `Unknown` | `MIT` | 53 |
| 🆕 💎 🧠 [getagentseal/awesome-mcp-security](https://github.com/getagentseal/awesome-mcp-security) | Security scores for 800+ MCP servers. 9 analyzers scan for prompt injection, toxic flows, and attack surface risks. U... | 39 | +0 / +0 | `Awesome List` | `Unknown` | `NOASSERTION` | 53 |
| 🆕 💎 🧠 [FuzzingLabs/mcp-security-hub](https://github.com/FuzzingLabs/mcp-security-hub) | A growing collection of MCP servers bringing offensive security tools to AI assistants. Nmap, Ghidra, Nuclei, SQLMap,... | 793 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 53 |
| 🆕 💎 🧠 [ax128/AegisGate](https://github.com/ax128/AegisGate) | Open-source security gateway for LLM APIs — prompt injection detection, PII redaction, dangerous response sanitizatio... | 68 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 52 |
| 🆕 💎 🧠 [sunyinggang/LLM-Agent-Security-Survey](https://github.com/sunyinggang/LLM-Agent-Security-Survey) | The official GitHub page for the survey paper "A Survey on the Unique Security of Autonomous and Collaborative LLM Ag... | 13 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 52 |
| 🆕 💎 🧠 ✅ [google/mcp-security](https://github.com/google/mcp-security) | No description. | 530 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 52 |
| 🆕 💎 🧠 [sahiloj/MCPScan](https://github.com/sahiloj/MCPScan) | Offensive MCP server auditor: detects tool poisoning, credential leaks, RCE vectors, SSRF, session hijacking, and sup... | 25 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 51 |
| 🆕 🏆 💎 [mksglu/context-mode](https://github.com/mksglu/context-mode) | Context window optimization for AI coding agents. Sandboxes tool output (98% reduction), persists session memory, and... | 24,078 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `NOASSERTION` | 51 |
| 🆕 💎 🧠 [Helixar-AI/sentinel](https://github.com/Helixar-AI/sentinel) | MCP security scanner by Helixar | 10 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 50 |
| 🆕 💎 🧠 [slowmist/MCP-Security-Checklist](https://github.com/slowmist/MCP-Security-Checklist) | A comprehensive security checklist for MCP-based AI tools. Built by SlowMist to safeguard LLM plugin ecosystems. | 831 | +0 / +0 | `Guide / Standard` | `Unknown` | `MIT` | 50 |
| 🆕 💎 🧠 ✅ [Azure-Samples/sherpa](https://github.com/Azure-Samples/sherpa) | A guided lab for MCP security and best practices | 29 | +0 / +0 | `Tool / Framework` | `PowerShell` | `MIT` | 50 |
| 🆕 🧠 [Htunn/offensive-ai](https://github.com/Htunn/offensive-ai) | offsec-ai is a Python library and CLI that combines classic network reconnaissance with modern AI/LLM security testing. | 32 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 49 |
| 🆕 🧠 [GenAI-Security-Project/GenAI-Security-Advisor](https://github.com/GenAI-Security-Project/GenAI-Security-Advisor) | AI agent/skill grounded in the OWASP GenAI Security Project's published research (LLM Top 10, Agentic Top 10, Data Se... | 10 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 48 |
| 🆕 🧠 [StackOneHQ/defender](https://github.com/StackOneHQ/defender) | Open source prompt injection protection for Agents calling tools (via MCP, CLI or direct function calling). Detect an... | 124 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `Apache-2.0` | 48 |
| 🆕 🧠 [Helixar-AI/mcp-security-checklist](https://github.com/Helixar-AI/mcp-security-checklist) | MCP is being adopted rapidly. Security guidance is lagging behind. This checklist gives security engineers, platform ... | 22 | +0 / +0 | `Guide / Standard` | `Unknown` | `MIT` | 47 |
| 🆕 🧠 [aminrj-labs/mcp-attack-labs](https://github.com/aminrj-labs/mcp-attack-labs) | Hands-on offensive-security labs for AI agents — MCP tool poisoning, cross-server shadowing, RAG injection, agentic m... | 21 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 47 |
| 🆕 🧠 [hashgraph-online/hol-guard](https://github.com/hashgraph-online/hol-guard) | Open-source antivirus for AI agents: block risky tools, secret access, prompt injection, malicious packages, MCP serv... | 668 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 47 |
| 🆕 🧠 ✅ [gl0di/clawseccheck](https://github.com/gl0di/clawseccheck) | 🦞 Local, read-only security audit for your OpenClaw agent. Finds config, prompt-injection and supply-chain risks, wat... | 58 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 47 |
| 🆕 🧠 [AIM-Intelligence/awesome-mcp-security](https://github.com/AIM-Intelligence/awesome-mcp-security) | Security Threats related with MCP (Model Context Protocol), MCP Servers and more | 51 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 45 |
| 🆕 🧠 [hackersatyamrastogi/pentesting-cyber-mcp](https://github.com/hackersatyamrastogi/pentesting-cyber-mcp) | 🔐 50+ MCP Security Servers for AI-Powered Pentesting \| Integrate Nmap, Burp Suite, Nuclei, Shodan, BloodHound, Semgr... | 29 | +0 / +0 | `Research / Benchmark` | `JavaScript` | `MIT` | 45 |
| 🆕 🧠 [nayangoel/PromptInjector](https://github.com/nayangoel/PromptInjector) | A comprehensive defensive security testing tool for AI systems. PromptInjector helps identify prompt injection vulner... | 100 | +0 / +0 | `Guide / Standard` | `Python` | `Unknown` | 45 |
| 🆕 🧠 [FarzamHabibi/pre-production-checklist](https://github.com/FarzamHabibi/pre-production-checklist) | Pre-production checklists for solo founders. 4,394 items, 87% portable to any stack, 26 stack supplements — plus AI/a... | 15 | +0 / +0 | `Tool / Framework` | `JavaScript` | `NOASSERTION` | 44 |
| 🆕 🧠 [arcjet/arcjet-js](https://github.com/arcjet/arcjet-js) | Runtime security for AI apps and agents: prompt injection detection, tool-call authorization, sensitive-data redactio... | 685 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `Apache-2.0` | 44 |
| 🆕 🧠 [82ch/MCP-Dandan](https://github.com/82ch/MCP-Dandan) | MCP Security Solution for Agentic AI — real-time proxying, behavior analysis, and malicious tool detection | 66 | +0 / +0 | `Awesome List` | `Python` | `MIT` | 43 |
| 🆕 🧠 [putervision/spc](https://github.com/putervision/spc) | High-performance zero-dependency static analysis tool enforcing NASA Power of Ten rules across 20 programming languag... | 39 | +0 / +0 | `Tool / Framework` | `JavaScript` | `MIT` | 43 |
| 🆕 🧠 [bountyyfi/mcp-watchdog](https://github.com/bountyyfi/mcp-watchdog) | MCP security proxy that sits between AI coding assistants and MCP servers, detecting and blocking all known MCP attac... | 25 | +0 / +0 | `Tool / Framework` | `Python` | `Unknown` | 43 |
| 🆕 🧠 [seshan-arunagiri/AEGIS](https://github.com/seshan-arunagiri/AEGIS) | AgentShield is an AI security middleware that protects LLM agents from MCP tool poisoning and prompt injection attack... | 13 | +0 / +0 | `Tool / Framework` | `TypeScript` | `Unknown` | 42 |
| 🆕 🧠 [Unclecheng-li/DeepSec](https://github.com/Unclecheng-li/DeepSec) | DeepSec — AI Security Offense & Defense Platform. Shield audits AI-generated code for hallucinated packages, missing ... | 446 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 42 |
| 🆕 🧠 [makalin/SecureMCP](https://github.com/makalin/SecureMCP) | SecureMCP is a security auditing tool designed to detect vulnerabilities and misconfigurations in applications using ... | 140 | +0 / +0 | `Tool / Framework` | `Go` | `MIT` | 41 |
| 🆕 🧠 [cybozu/prompt-hardener](https://github.com/cybozu/prompt-hardener) | Prompt Hardener analyzes prompt-injection-originated risk in LLM-based agents and applications. | 54 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 40 |
| 🆕 🧠 [AgentsID-dev/agentsid-scanner](https://github.com/AgentsID-dev/agentsid-scanner) | Security scanner for MCP servers. Grades auth, permissions, injection risks, and tool safety. The Lighthouse of agent... | 25 | +0 / +0 | `Research / Benchmark` | `JavaScript` | `MIT` | 39 |
| 🆕 🧠 [msaleme/red-team-blue-team-agent-fabric](https://github.com/msaleme/red-team-blue-team-agent-fabric) | AI agent security harness, adversarial testing: 640 executable tests in the v4.25.0 release, across MCP, A2A, x402/L4... | 31 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 39 |
| 🆕 🧠 [GhostWolfLab/AI_Security_Top](https://github.com/GhostWolfLab/AI_Security_Top) | 打造最强的AI安全文档 | 151 | +0 / +0 | `Guide / Standard` | `Unknown` | `MIT` | 39 |
| 🆕 🧠 [purpleroc/mcp-security-inspector](https://github.com/purpleroc/mcp-security-inspector) | 一个用于检测Model Context Protocol (MCP)安全性的Chrome扩展工具。 | 37 | +0 / +0 | `Tool / Framework` | `TypeScript` | `Unknown` | 38 |
| 🆕 🧠 [ryanfren/AgentCloak](https://github.com/ryanfren/AgentCloak) | Secure email proxy for AI agents — content filtering, PII redaction, and prompt injection detection over MCP | 12 | +0 / +0 | `Tool / Framework` | `TypeScript` | `NOASSERTION` | 38 |
| 🆕 🧠 [princezuda/safestclaw](https://github.com/princezuda/safestclaw) | Safestclaw is the alternative to openclaw.. You can naturally chat with it via text and voice, and you can choose not... | 280 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 38 |
| 🆕 ✅ [aws-samples/sample-mcp-security-scanner](https://github.com/aws-samples/sample-mcp-security-scanner) | This pattern describes how to implement a Model Context Protocol (MCP) server that integrates three industry-standard... | 16 | +0 / +0 | `Tool / Framework` | `Python` | `MIT-0` | 37 |
| 🆕 ✅ [1Password/SCAM](https://github.com/1Password/SCAM) | SCAM - Security Comprehension Awareness Measure \| Open-source benchmark that tests AI agents' security awareness dur... | 136 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 37 |
| 🆕 🧠 [rsfl/splunk-mcp-llm-siemulator](https://github.com/rsfl/splunk-mcp-llm-siemulator) | A Docker lab integrating Splunk SIEM with Ollama LLM via MCP for AI security operations. Features Promptfoo OWASP eva... | 34 | +0 / +0 | `Research / Benchmark` | `PowerShell` | `MIT` | 36 |
| 🆕 🧠 [arcjet/arcjet-py](https://github.com/arcjet/arcjet-py) | Runtime security for AI apps and agents: prompt injection detection, tool-call authorization, sensitive-data redactio... | 32 | +0 / +0 | `Tool / Framework` | `Python` | `Apache-2.0` | 36 |
| 🆕 🧠 [ismailkho/awesome-mcp-servers-security](https://github.com/ismailkho/awesome-mcp-servers-security) | A curated list of Model Context Protocol (MCP) servers with real-time security scores and cloud skills. | 24 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 36 |
| 🆕 🧠 [Synvoya/codeinspectus](https://github.com/Synvoya/codeinspectus) | Local-first MCP security scanner for AI-generated apps. Scan → fix → rescan from Claude Code, Cursor, Codex, and othe... | 47 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `Apache-2.0` | 35 |
| 🆕 [BIGPPWONG/EdgeBox](https://github.com/BIGPPWONG/EdgeBox) | A fully-featured, GUI-powered local LLM Agent sandbox with complete MCP protocol support. Features both CLI and full ... | 212 | +0 / +0 | `Tool / Framework` | `TypeScript` | `GPL-3.0` | 34 |
| 🆕 [slowmist/slowmist-agent-security](https://github.com/slowmist/slowmist-agent-security) | SlowMist Agent Security Skill: A comprehensive security review framework for AI agents operating in adversarial envir... | 507 | +0 / +0 | `Research / Benchmark` | `Unknown` | `MIT` | 33 |
| 🆕 [SnailSploit/Burp-MCP-Security-Analysis-Toolkit](https://github.com/SnailSploit/Burp-MCP-Security-Analysis-Toolkit) | Burp MCP Security Analysis Toolkit | 17 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 32 |
| 🆕 [Aembit/agentic-ai-security-starter-kit](https://github.com/Aembit/agentic-ai-security-starter-kit) | Working code examples to defend against Agentic AI threats: prompt injection detection, Claude Code hooks, OPA polici... | 14 | +0 / +0 | `Guide / Standard` | `Python` | `MIT` | 31 |
| 🆕 [Nova-Hunting/nova-tracer](https://github.com/Nova-Hunting/nova-tracer) | NOVA - Claude Code Protection System against prompt injection attacks | 95 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 31 |
| 🆕 [putervision/WebCrypt](https://github.com/putervision/WebCrypt) | Zero-dependency Web Crypto suite & Model Context Protocol (MCP) server for AES-256-GCM, RSA-4096, and AI agent security. | 28 | +0 / +0 | `Tool / Framework` | `JavaScript` | `MIT` | 31 |
| 🆕 🧠 [mcp-security-standard/mcp-server-security-standard](https://github.com/mcp-security-standard/mcp-server-security-standard) | MCP Server Security Standard (MSSS): an open, testable security control standard for certifying MCP servers, with lev... | 74 | +0 / +0 | `Tool / Framework` | `Unknown` | `CC-BY-SA-4.0` | 30 |
| 🆕 🧠 [Siddharth-1001/llm-security-scanner](https://github.com/Siddharth-1001/llm-security-scanner) | A lightweight tool that scans LLM-integrated codebases for OWASP LLM Top 10 vulnerabilities — prompt injection patter... | 15 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 30 |
| 🆕 [vaultmcp/vault](https://github.com/vaultmcp/vault) | MCP prompt-injection scanning proxy — runtime security for MCP tool responses | 101 | +0 / +0 | `Research / Benchmark` | `Solidity` | `MIT` | 30 |
| 🆕 [olanokhin/openresearch-mcp](https://github.com/olanokhin/openresearch-mcp) | Zero-auth, security-hardened cross-domain research MCP server: web, academic, GitHub, news, weather, macro/finance, S... | 11 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 30 |
| 🆕 [Clear-Capabilities/agentic-security](https://github.com/Clear-Capabilities/agentic-security) | Build faster with an Agentic Workforce. Safe, secure, and compliant is now the default. | 117 | +0 / +0 | `Tool / Framework` | `JavaScript` | `NOASSERTION` | 30 |
| 🆕 [gensecaihq/MCP-Developer-SubAgent](https://github.com/gensecaihq/MCP-Developer-SubAgent) | A specialized framework for Model Context Protocol (MCP) development featuring 8 Claude Code sub-agents, security hoo... | 28 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 29 |
| 🆕 [slowmist/MasterMCP](https://github.com/slowmist/MasterMCP) | A demonstration toolkit revealing potential security vulnerabilities in MCP (Model Context Protocol) frameworks throu... | 98 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 29 |
| 🆕 [DFKHelper/token-goat](https://github.com/DFKHelper/token-goat) | Token burn reducer and focus keeper for Claude Code, Codex, Copilot, Gemini CLI, and more: surgical read hints, PDF/O... | 126 | +0 / +0 | `Tool / Framework` | `TypeScript` | `NOASSERTION` | 29 |
| 🆕 [mathematic-inc/earl](https://github.com/mathematic-inc/earl) | Secure CLI proxy for AI agents — HCL-defined operation templates with OS keychain secrets, MCP integration, and promp... | 113 | +0 / +0 | `Tool / Framework` | `Rust` | `Apache-2.0` | 29 |
| 🆕 [Repello-AI/mcp-exploit-demo](https://github.com/Repello-AI/mcp-exploit-demo) | This repository demonstrates a security vulnerability in MCP (Model Context Protocol ) servers that allows for remote... | 25 | +0 / +0 | `Guide / Standard` | `Python` | `Unknown` | 28 |
| 🆕 [ChuprinaDaria/Vibecode-Cleaner-Fartrun](https://github.com/ChuprinaDaria/Vibecode-Cleaner-Fartrun) | Not another AI reviewing AI. Rust-powered code scanner that actually reads your codebase — zero extra tokens consumed... | 139 | +0 / +0 | `Awesome List` | `Python` | `NOASSERTION` | 27 |
| 🆕 [DMontgomery40/mcp-security-scanner](https://github.com/DMontgomery40/mcp-security-scanner) | A security vulnerability scanner built with MCP plugins | 12 | +0 / +0 | `Research / Benchmark` | `JavaScript` | `Unknown` | 26 |
| 🆕 [ModelContextProtocol-Security/modelcontextprotocol-security.io](https://github.com/ModelContextProtocol-Security/modelcontextprotocol-security.io) | Official website and documentation hub for the Model Context Protocol Security initiative. Provides security guidance... | 26 | +0 / +0 | `Research / Benchmark` | `Unknown` | `CC0-1.0` | 26 |
| 🆕 [onsails/right-agent](https://github.com/onsails/right-agent) | agents that run on your claude subscription. multi-agent, sandboxed by default. polished telegram UI. | 35 | +0 / +0 | `Guide / Standard` | `Rust` | `Apache-2.0` | 25 |
| 🆕 [khendzel/skills-janitor](https://github.com/khendzel/skills-janitor) | Tinder for your Claude Code skills, subagents and MCP servers. Swipe away what wastes context, scan for prompt inject... | 121 | +0 / +0 | `Tool / Framework` | `Shell` | `MIT` | 25 |
| 🆕 [General-Analysis/mcp-guard](https://github.com/General-Analysis/mcp-guard) | MCP Guard secures your MCP client from prompt injection attacks and more. | 55 | +0 / +0 | `Tool / Framework` | `TypeScript` | `MIT` | 24 |
| 🆕 [shriramkv/mcp-server-audit-A-Static-Security-and-Hygiene-Audit-CLI-for-MCP-Servers](https://github.com/shriramkv/mcp-server-audit-A-Static-Security-and-Hygiene-Audit-CLI-for-MCP-Servers) | A CI-friendly open-source CLI that statically audits MCP servers before they are wired into AI agents. It flags tool ... | 23 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 24 |
| 🆕 [Srajangpt1/ai-security-crew](https://github.com/Srajangpt1/ai-security-crew) | MCP to put security context while vibe coding | 68 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 23 |
| 🆕 [spring-ai-community/mcp-security](https://github.com/spring-ai-community/mcp-security) | Spring Security Configuration for MCP | 115 | +0 / +0 | `Tool / Framework` | `Java` | `Apache-2.0` | 23 |
| 🆕 [dan1t0/gophish-mcp](https://github.com/dan1t0/gophish-mcp) | A lightweight toolkit of scripts and utilities to automate Gophish campaigns, streamline phishing assessments, and sp... | 21 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 22 |
| 🆕 [azukaar/plurality](https://github.com/azukaar/plurality) | Open-source AI virtual assistant agent sandbox/runner with native UI client (OpenClaw/Chatbot alternative) | 35 | +0 / +0 | `Tool / Framework` | `Dart` | `MIT` | 21 |
| 🆕 [Ephemeral-AI-Lab/ephemeral-sandbox](https://github.com/Ephemeral-AI-Lab/ephemeral-sandbox) | Open-source agent sandbox infrastructure for parallel coding agents: isolated workspaces, MCP/CLI control, observabil... | 104 | +0 / +0 | `Research / Benchmark` | `Rust` | `MIT` | 21 |
| 🆕 [qianniuspace/mcp-security-audit](https://github.com/qianniuspace/mcp-security-audit) | A powerful MCP (Model Context Protocol) Server that audits npm package dependencies for security vulnerabilities. Bui... | 57 | +0 / +0 | `Tool / Framework` | `TypeScript` | `MIT` | 20 |
| 🆕 [realchendahuang/OMPChamber](https://github.com/realchendahuang/OMPChamber) | 多智能体协同、自动化管线沙盒与商业验证试验仓 / Multi-agent sandbox and execution validation chamber | 55 | +0 / +0 | `Tool / Framework` | `TypeScript` | `MIT` | 20 |
| 🆕 [fr0gger/MCP_Security](https://github.com/fr0gger/MCP_Security) | This is a repository to experiment with MCP for security | 51 | +0 / +0 | `Tool / Framework` | `Python` | `Unknown` | 20 |
| 🆕 [thomaspeklak/agent-sandbox](https://github.com/thomaspeklak/agent-sandbox) | Sandbox for agents with podman | 16 | +0 / +0 | `Tool / Framework` | `Rust` | `MIT` | 20 |
| 🆕 [mihirahuja1/agentnestOSS](https://github.com/mihirahuja1/agentnestOSS) | Open Source agent Sandboxes | 13 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 19 |
| 🆕 [andrasfe/vulnicheck](https://github.com/andrasfe/vulnicheck) | Python vulnerability scanner & MCP security toolkit. Real-time dependency checking via OSV/NVD/GitHub Advisory DBs, D... | 11 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 18 |

[Back to top](#ai-security-repository-radar)

---

<a id="model-ml-security"></a>

## 🧬 Model / ML Security

**Repositories:** 58  
**Labs:** 12  
**Stars:** 21,085  
**7-day growth:** +0

### Benchmarks / Eval

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [corca-ai/awesome-llm-security](https://github.com/corca-ai/awesome-llm-security) | A curation of awesome tools, documents and projects about LLM Security. | 1,702 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 78 |
| 🆕 💎 🧠 [CryptoAILab/Awesome-LM-SSP](https://github.com/CryptoAILab/Awesome-LM-SSP) | A reading list for large models safety, security, and privacy (including Awesome LLM Security, Safety, etc.). | 2,080 | +0 / +0 | `Awesome List` | `Unknown` | `Apache-2.0` | 75 |
| 🆕 🧠 [unica-mlsec/mlsec](https://github.com/unica-mlsec/mlsec) | PhD/MSc course on Machine Learning Security (Univ. Cagliari) | 232 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `Unknown` | 37 |
| 🆕 🧠 [byerose/Awesome-Foundation-Model-Security](https://github.com/byerose/Awesome-Foundation-Model-Security) | A curated list of trustworthy Generative AI papers. Daily updating... | 77 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 34 |
| 🆕 [ylhz/Adversarial_Attacks_and_Defense_NeurIPS2022](https://github.com/ylhz/Adversarial_Attacks_and_Defense_NeurIPS2022) | A list of papers in NeurIPS 2022 related to adversarial attack and defense / AI security. | 78 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 31 |
| 🆕 [AdamtayZzz/AI-security-related-paper-list](https://github.com/AdamtayZzz/AI-security-related-paper-list) | Papers I have collected and read in undergraduate and graduate period | 59 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 27 |
| 🆕 [NetSPI/Open-LLM-Security-Benchmark](https://github.com/NetSPI/Open-LLM-Security-Benchmark) | No description. | 24 | +0 / +0 | `Research / Benchmark` | `HTML` | `NOASSERTION` | 26 |
| 🆕 [zaixizhang/FLDetector](https://github.com/zaixizhang/FLDetector) | The official code of KDD22 paper "FLDetecotor: Defending Federated Learning Against Model Poisoning Attacks via Detec... | 87 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 23 |
| 🆕 [PurduePAML/Machine-Learning-Security-Seminar](https://github.com/PurduePAML/Machine-Learning-Security-Seminar) | Machine Learning & Security Seminar @Purdue University | 26 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 23 |
| 🆕 [jeremy313/FL-WBC](https://github.com/jeremy313/FL-WBC) | Official implementation of "FL-WBC: Enhancing Robustness against Model Poisoning Attacks in Federated Learning from a... | 44 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 22 |
| 🆕 [xyq7/PoisonedFL](https://github.com/xyq7/PoisonedFL) | CVPR 2025, Model Poisoning Attacks to Federated Learning via Multi-Round Consistency | 19 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 21 |
| 🆕 [xyq7/FedREDefense](https://github.com/xyq7/FedREDefense) | The official code for ICML 2024 "FedREDefense: Defending against Model Poisoning Attacks for Federated Learning using... | 31 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 20 |
| 🆕 [vrt1shjwlkr/NDSS21-Model-Poisoning](https://github.com/vrt1shjwlkr/NDSS21-Model-Poisoning) | Code for NDSS 2021 Paper "Manipulating the Byzantine: Optimizing Model Poisoning Attacks and Defenses Against Federat... | 153 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `Unknown` | 18 |
| 🆕 [mindspore-ai/mindarmour](https://github.com/mindspore-ai/mindarmour) | A tool box for MindSpore users to enhance model security and trustworthiness. | 92 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 18 |
| 🆕 [PacktPublishing/Machine-Learning-Security-Principles](https://github.com/PacktPublishing/Machine-Learning-Security-Principles) | Machine Learning Security Principles, published by Packt | 23 | +0 / +0 | `Research / Benchmark` | `Jupyter Notebook` | `MIT` | 17 |
| 🆕 [rdz98/FedRecAttack](https://github.com/rdz98/FedRecAttack) | Model Poisoning Attack to Federated Recommendation | 33 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 17 |
| 🆕 [jeremy313/non-iid-dataset-for-personalized-federated-learning](https://github.com/jeremy313/non-iid-dataset-for-personalized-federated-learning) | Official implementation of "FL-WBC: Enhancing Robustness against Model Poisoning Attacks in Federated Learning from a... | 87 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 16 |
| 🆕 [inspire-group/ModelPoisoning](https://github.com/inspire-group/ModelPoisoning) | Code for "Analyzing Federated Learning through an Adversarial Lens" https://arxiv.org/abs/1811.12470 | 152 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 16 |

### Dashboard / Platform

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 ✅ [aws-samples/sample-aiml-security-assessment](https://github.com/aws-samples/sample-aiml-security-assessment) | AI/ML and Generative AI Security Assessment Framework for AWS. Automatically audit Amazon Bedrock , SageMaker ,AgentC... | 46 | +0 / +0 | `Research / Benchmark` | `HTML` | `MIT-0` | 68 |

### Framework / Library

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🏆 💎 🧠 [Trusted-AI/adversarial-robustness-toolbox](https://github.com/Trusted-AI/adversarial-robustness-toolbox) | Adversarial Robustness Toolbox (ART) - Python Library for Machine Learning Security - Evasion, Poisoning, Extraction,... | 6,245 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 66 |
| 🆕 💎 🧠 [DeepSpaceHarbor/Awesome-AI-Security](https://github.com/DeepSpaceHarbor/Awesome-AI-Security) | :file_folder: #AISecurity | 1,675 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 60 |
| 🆕 [AnchoretY/AI_Security_Library](https://github.com/AnchoretY/AI_Security_Library) | Ai与Web安全相关资料的总结库，包括认为写的比较好的一些博客、项目、数据等 | 84 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 26 |
| 🆕 [drmhrehman/trustfed](https://github.com/drmhrehman/trustfed) | This repository presents a blockchain-based framework, TrustFed, for Cross-Device Federated Learning systems to detec... | 13 | +0 / +0 | `Tool / Framework` | `Jupyter Notebook` | `GPL-3.0` | 11 |

### General

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 ✅ [OWASP/www-project-machine-learning-security-top-10](https://github.com/OWASP/www-project-machine-learning-security-top-10) | OWASP Machine Learning Security Top 10 Project | 106 | +0 / +0 | `Project` | `HTML` | `NOASSERTION` | 34 |
| 🆕 [wearetyomsmnv/AI-LLM-ML_security_study_map](https://github.com/wearetyomsmnv/AI-LLM-ML_security_study_map) | Do you want to learn AI Security but don't know where to start ? Take a look at this map. | 29 | +0 / +0 | `Guide / Standard` | `Unknown` | `Unknown` | 26 |
| 🆕 [RiskySignal/record_what_i_read](https://github.com/RiskySignal/record_what_i_read) | AI Model Security Reading Notes | 40 | +0 / +0 | `Awesome List` | `Unknown` | `Unknown` | 25 |
| 🆕 [fabriciojoc/2021-Machine-Learning-Security-Evasion-Competition](https://github.com/fabriciojoc/2021-Machine-Learning-Security-Evasion-Competition) | 2021 Machine Learning Security Evasion Competition | 14 | +0 / +0 | `Guide / Standard` | `Python` | `MIT` | 17 |
| 🆕 [williamdevena/Defending-federated-learning-system](https://github.com/williamdevena/Defending-federated-learning-system) | Implementation of a client reputation, gradient checking and homomorphic encryption mechanism to defend a federated l... | 17 | +0 / +0 | `Project` | `Python` | `Unknown` | 15 |

### Jailbreak Testing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [PromptLabs/Prompt-Hacking-Resources](https://github.com/PromptLabs/Prompt-Hacking-Resources) | A list of curated resources for people interested in AI Red Teaming, Jailbreaking, and Prompt Injection | 727 | +0 / +0 | `Awesome List` | `Unknown` | `MIT` | 70 |
| 🆕 🧠 [Zhou-Zi7/Awesome-AI-Security-BIG4](https://github.com/Zhou-Zi7/Awesome-AI-Security-BIG4) | This Github repository summarizes a list of research papers on AI security from the four top academic conferences. | 186 | +0 / +0 | `Awesome List` | `Unknown` | `Apache-2.0` | 37 |

### LLM Labs

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 🧪 [regaan/basilisk](https://github.com/regaan/basilisk) | Basilisk — Open-source AI red teaming framework with genetic prompt evolution. Automated LLM security testing for GPT... | 31 | +0 / +0 | `Lab` | `Python` | `NOASSERTION` | 84 |
| 🆕 💎 🧠 🧪 [Mr-Infect/AI-penetration-testing](https://github.com/Mr-Infect/AI-penetration-testing) | AI/ML/LLM Penetration Testing Toolkit by Mr-Infect — the #1 GitHub resource for AI security, red teaming, and adversa... | 288 | +0 / +0 | `Lab` | `Unknown` | `Unknown` | 84 |
| 🆕 💎 🧠 🧪 [CyberSunil/LLMVault](https://github.com/CyberSunil/LLMVault) | An intentionally vulnerable OWASP LLM Top 10 training platform for AI Security, Prompt Injection, RAG Security, Agent... | 326 | +0 / +0 | `Lab` | `Python` | `MIT` | 82 |
| 🆕 💎 🧠 🧪 ✅ [microsoft/AI-Red-Teaming-Playground-Labs](https://github.com/microsoft/AI-Red-Teaming-Playground-Labs) | AI Red Teaming playground labs to run AI Red Teaming trainings including infrastructure. | 2,080 | +0 / +0 | `Lab` | `TypeScript` | `MIT` | 81 |
| 🆕 💎 🧠 🧪 [Panda1847/ai-security-lab](https://github.com/Panda1847/ai-security-lab) | 🤖 Advanced AI/LLM Security Testing Framework. Includes 50+ jailbreak techniques, prompt injection tools, and automate... | 10 | +0 / +0 | `Lab` | `Unknown` | `MIT` | 80 |
| 🆕 💎 🧠 🧪 [schwartz1375/genai-security-training](https://github.com/schwartz1375/genai-security-training) | No description. | 461 | +0 / +0 | `Lab` | `Jupyter Notebook` | `NOASSERTION` | 76 |
| 🆕 💎 🧠 🧪 [0xSojalSec/Awesome-AI-Security](https://github.com/0xSojalSec/Awesome-AI-Security) | Resources, research, and tools for securing AI systems | 31 | +0 / +0 | `Lab` | `Unknown` | `Unknown` | 65 |
| 🆕 💎 🧠 🧪 [ErdemOzgen/RedAiRange](https://github.com/ErdemOzgen/RedAiRange) | A professional AI security range for red teaming, vulnerability research, defensive validation, and hands-on AI/ML se... | 130 | +0 / +0 | `Lab` | `Jupyter Notebook` | `MIT` | 60 |
| 🆕 💎 🧠 🧪 [aligorithm/Zero-Health](https://github.com/aligorithm/Zero-Health) | Zero trust. Zero security. Total exposure. A deliberately vulnerable health tech platform with AI Chatbot for learnin... | 48 | +0 / +0 | `Lab` | `JavaScript` | `MIT` | 52 |
| 🆕 🧠 🧪 [0x4D31/airt](https://github.com/0x4D31/airt) | AIRT — A free, open-source AI Red Teaming course with 8 modules and hands-on Docker labs. Built with Perplexity Compu... | 230 | +0 / +0 | `Lab` | `HTML` | `Unknown` | 45 |
| 🆕 🧠 🧪 [HumanCompatibleAI/tensor-trust](https://github.com/HumanCompatibleAI/tensor-trust) | A prompt injection game to collect data for robust ML research | 78 | +0 / +0 | `Lab` | `Python` | `BSD-2-Clause` | 42 |

### ML Labs

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧪 [orcasecurity-research/AIGoat](https://github.com/orcasecurity-research/AIGoat) | AIGoat: A deliberately Vulnerable AI Infrastructure. Learn AI security through solving our challenges. | 290 | +0 / +0 | `Lab` | `CSS` | `Apache-2.0` | 36 |

### Model Scanning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [TravHaran/Im.Primo](https://github.com/TravHaran/Im.Primo) | TrueDepth 3D model scanning with wireless 3D Printing | 15 | +0 / +0 | `Tool / Framework` | `Python` | `Unknown` | 8 |

### Prompt Injection Defense

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [Shiva108/ai-llm-red-team-handbook](https://github.com/Shiva108/ai-llm-red-team-handbook) | AI / LLM Red Team Field Manual & Consultant’s Handbook | 335 | +0 / +0 | `Tool / Framework` | `Python` | `NOASSERTION` | 64 |
| 🆕 🧠 [briland/LLM-security-and-privacy](https://github.com/briland/LLM-security-and-privacy) | LLM security and privacy | 54 | +0 / +0 | `Awesome List` | `TeX` | `Unknown` | 46 |

### RAG Poisoning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [AI-Security-Research-Group/LLM-Attacks](https://github.com/AI-Security-Research-Group/LLM-Attacks) | Comprehensive taxonomy of AI security vulnerabilities, LLM adversarial attacks, prompt injection techniques, and mach... | 20 | +0 / +0 | `Project` | `Unknown` | `GPL-3.0` | 75 |
| 🆕 💎 🧠 [christiancscott/awesome-LLM-security](https://github.com/christiancscott/awesome-LLM-security) | A compendium of threat modeling and security testing resources for LLMs and GenAI agents | 21 | +0 / +0 | `Awesome List` | `Unknown` | `AGPL-3.0` | 61 |
| 🆕 💎 🧠 [perplext/LLMrecon](https://github.com/perplext/LLMrecon) | Enterprise-grade LLM security testing framework implementing OWASP LLM Top 10 with advanced prompt injection, jailbre... | 17 | +0 / +0 | `Tool / Framework` | `Go` | `MIT` | 59 |
| 🆕 💎 🧠 ✅ [neveratech/owasp-llm-top10-tr](https://github.com/neveratech/owasp-llm-top10-tr) | OWASP GenAI LLM Top 10 2026 — gayriresmî Türkçe çeviri (CC BY-SA 4.0) | 11 | +0 / +0 | `Project` | `Unknown` | `NOASSERTION` | 57 |
| 🆕 💎 🧠 [precize/Agentic-AI-Top10-Vulnerability](https://github.com/precize/Agentic-AI-Top10-Vulnerability) | Top 10 for Agentic AI (AI Agent Security) serves as the core for OWASP and CSA Red teaming work | 202 | +0 / +0 | `Tool / Framework` | `Unknown` | `Unknown` | 55 |
| 🆕 🧠 [SecNode/AISecLists](https://github.com/SecNode/AISecLists) | AISecLists - Your AI Red Teaming Arsenal. Discover a curated collection of prompt lists for diverse AI security asses... | 16 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 46 |
| 🆕 🧠 [ZhengyuZhao/AI-Security-and-Privacy-Events](https://github.com/ZhengyuZhao/AI-Security-and-Privacy-Events) | A curated list of academic events on AI Security & Privacy | 168 | +0 / +0 | `Research / Benchmark` | `Unknown` | `MIT` | 46 |
| 🆕 [abhisheksaxena1998/Malicious-Urlv5](https://github.com/abhisheksaxena1998/Malicious-Urlv5) | A multi-layered and multi-tiered Machine Learning security solution, it supports always on detection system, Django R... | 46 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 25 |

### Sandboxing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [openappsec/openappsec](https://github.com/openappsec/openappsec) | open-appsec is a machine learning security engine that preemptively and automatically prevents threats against Web Ap... | 1,713 | +0 / +0 | `Guide / Standard` | `C++` | `Apache-2.0` | 60 |
| 🆕 ✅ [GoogleCloudPlatform/activation-model-scanner](https://github.com/GoogleCloudPlatform/activation-model-scanner) | Verify language model safety before deployment by analyzing activation patterns | 33 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 41 |
| 🆕 [openappsec/attachment](https://github.com/openappsec/attachment) | Attachments connect between processes that provide HTTP data and the open-appsec Agent security logic. | 32 | +0 / +0 | `Guide / Standard` | `C` | `Apache-2.0` | 21 |
| 🆕 [EthicalML/fml-security](https://github.com/EthicalML/fml-security) | Practical examples of "Flawed Machine Learning Security" together with ML Security best practice across the end to en... | 125 | +0 / +0 | `Awesome List` | `Python` | `Unknown` | 16 |

### Tool / Function Calling

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [AiGptCode/Ai-Security-URL](https://github.com/AiGptCode/Ai-Security-URL) | functions to exploit common web application vulnerabilities such as SQL Injection, Cross-Site Scripting (XSS), Server... | 93 | +0 / +0 | `Project` | `Python` | `Unknown` | 23 |

[Back to top](#ai-security-repository-radar)

---

<a id="other-ai-security"></a>

## 🔐 Other AI Security

**Repositories:** 6  
**Labs:** 0  
**Stars:** 2,258  
**7-day growth:** +0

### Benchmarks / Eval

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [ZiyueWang25/llm-security-challenge](https://github.com/ZiyueWang25/llm-security-challenge) | Can Large Language Models Solve Security Challenges? We test LLMs' ability to interact and break out of shell environ... | 13 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 14 |

### Dashboard / Platform

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [harishsg993010/crossbow-agent](https://github.com/harishsg993010/crossbow-agent) | world's first Opensource fully Autonomous AI Security Engineer | 245 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 26 |
| 🆕 [OTRF/GenAI-Security-Adventures](https://github.com/OTRF/GenAI-Security-Adventures) | No description. | 111 | +0 / +0 | `Project` | `Jupyter Notebook` | `MIT` | 22 |

### General

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 ✅ [OWASP/www-project-top-10-for-large-language-model-applications](https://github.com/OWASP/www-project-top-10-for-large-language-model-applications) | OWASP Top 10 for Large Language Model Apps (Part of the GenAI Security Project) | 1,419 | +0 / +0 | `Project` | `Python` | `NOASSERTION` | 58 |

### Jailbreak Testing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [Mai-xiyu/Codex-Prompt](https://github.com/Mai-xiyu/Codex-Prompt) | Rational engineering system prompt for Codex and LLMs — focused on accuracy, software engineering, AI, security resea... | 94 | +0 / +0 | `Research / Benchmark` | `Unknown` | `Unknown` | 23 |

### Tool / Function Calling

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [In3tinct/Androidmeda](https://github.com/In3tinct/Androidmeda) | AI Security tool to deobfuscate and find any potential vulnerabilities in android apps. | 376 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 29 |

[Back to top](#ai-security-repository-radar)

---

<a id="rag-security"></a>

## 📚 RAG Security

**Repositories:** 12  
**Labs:** 3  
**Stars:** 895  
**7-day growth:** +0

### Benchmarks / Eval

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧠 ✅ [promptfoo/promptfoo-action](https://github.com/promptfoo/promptfoo-action) | The GitHub Action for Promptfoo. Test your prompts, agents, and RAGs. AI Red teaming, pentesting, and vulnerability s... | 74 | +0 / +0 | `Research / Benchmark` | `TypeScript` | `MIT` | 49 |

### Jailbreak Testing

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 [olegnazarov/rag-security-scanner](https://github.com/olegnazarov/rag-security-scanner) | RAG/LLM Security Scanner identifies critical vulnerabilities in AI-powered applications, including chatbots, virtual ... | 75 | +0 / +0 | `Tool / Framework` | `Python` | `MIT` | 67 |
| 🆕 💎 🧠 [deadbits/vigil-llm](https://github.com/deadbits/vigil-llm) | ⚡ Vigil ⚡ Detect prompt injections, jailbreaks, and other potentially risky Large Language Model (LLM) inputs | 496 | +0 / +0 | `Research / Benchmark` | `Python` | `Apache-2.0` | 56 |

### LLM Labs

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 💎 🧠 🧪 [airtasystems/DVAIA-Damn-Vulnerable-AI-Application](https://github.com/airtasystems/DVAIA-Damn-Vulnerable-AI-Application) | Damn Vulnerable AI Application - For LLM Red Team Training. LLM testing, RAG testing, Multimodal testing, Agent testi... | 41 | +0 / +0 | `Lab` | `Python` | `Unknown` | 67 |
| 🆕 🧠 🧪 [wearetyomsmnv/Awesome-LLM-agent-Security](https://github.com/wearetyomsmnv/Awesome-LLM-agent-Security) | All about llm-agents security,attack,vulnerabilities and how to do them for cybersecurity. | 58 | +0 / +0 | `Lab` | `Unknown` | `Unlicense` | 47 |
| 🆕 🧠 🧪 [r00tb3/RAG-Poisoning-Lab](https://github.com/r00tb3/RAG-Poisoning-Lab) | RAG Poisoning Lab — Educational AI Security Exercise | 10 | +0 / +0 | `Lab` | `Python` | `Unknown` | 44 |

### RAG Poisoning

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 🧠 [prompt-security/RAG_Poisoning_POC](https://github.com/prompt-security/RAG_Poisoning_POC) | Stealthy Prompt Injection and Poisoning in RAG Systems via Vector Database Embeddings | 22 | +0 / +0 | `Research / Benchmark` | `Python` | `AGPL-3.0` | 49 |
| 🆕 🧠 [Yuning-J/CVE-KGRAG](https://github.com/Yuning-J/CVE-KGRAG) | Hybrid Knowledge Graph + RAG security intelligence platform for CVE analytics and semantic search. Llama3 integrated. | 21 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 36 |
| 🆕 [Mixture-of-RAGs-Security-Experts/MoRSE](https://github.com/Mixture-of-RAGs-Security-Experts/MoRSE) | MoRSE (Mixture of RAGs Security Experts), the first specialised AI chatbot for cybersecurity. MoRSE aims to provide c... | 16 | +0 / +0 | `Research / Benchmark` | `Python` | `Unknown` | 33 |
| 🆕 [SchwinnL/LLM_Embedding_Attack](https://github.com/SchwinnL/LLM_Embedding_Attack) | Code to conduct an embedding attack on LLMs | 34 | +0 / +0 | `Research / Benchmark` | `Python` | `MIT` | 17 |
| 🆕 [anshumanbh/cyber-safari](https://github.com/anshumanbh/cyber-safari) | A fun POC that is built to understand AI security agents. | 36 | +0 / +0 | `Project` | `Python` | `MIT` | 16 |

### Tool / Function Calling

| Repo | Description | Stars | Growth | Type | Language | License | Quality |
|---|---|---:|---:|---|---|---|---:|
| 🆕 [samuelberston/vuln-agent](https://github.com/samuelberston/vuln-agent) | LLM security agent capable of triaging vulnerabilities in source code using open source SAST/SCA | 12 | +0 / +0 | `Guide / Standard` | `Python` | `MIT` | 30 |

[Back to top](#ai-security-repository-radar)

---

